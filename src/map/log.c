...
static char log_picktype2char(e_log_pick_type type)
{
	...
	if ( type == LOG_FILTER_PRICE )
		return 'S'; //retoran valor desconocido pero sin debug.
...
void log_bg_kill(struct map_session_data* ssd, struct map_session_data* tsd, int skill)
{
...
void log_woe_kill(struct map_session_data* ssd, struct map_session_data* tsd, int skill)
{
	char esc_sname[NAME_LENGTH*2+1];
	char esc_tname[NAME_LENGTH*2+1];
