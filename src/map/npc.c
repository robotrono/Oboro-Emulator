...
int npc_cashshop_buylist(struct map_session_data *sd, int points, int count, unsigned short* item_list)
{
	...
	if( !nd || (nd->subtype != SPSHOP && nd->subtype != WOESHOP && nd->subtype != COSTUMESHOP && nd->subtype != CASHSHOP) )
		return 1;
...
int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points)
{
	...
	if( !nd || (nd->subtype != SPSHOP && nd->subtype != WOESHOP && nd->subtype != CASHSHOP && nd->subtype != COSTUMESHOP ) )
		return 1;
	...
	if( pc_isPremium(sd) && nd->subtype == CASHSHOP && battle_config.premium_discount > 0 )
	{
		price -= price * battle_config.premium_discount / 100;
		if( price <= 0 ) price = 1;
	}
	...
	if( !pet_create_egg(sd, nameid) )
	{
		item_tmp.nameid = nameid;
		item_tmp.identify = 1;
		// isaac xRO full woe cashshop as SSHOP
		if( nd->subtype == WOESHOP )
			return item_tmp.nameid;
		else if ( nd->subtype == COSTUMESHOP ) {
...
int npc_unload(struct npc_data* nd)
{
	...
	//isaac rathena unloadnpcfile adapt
	if( nd->path ) aFree(nd->path);

	if( ( nd->subtype == SHOP || nd->subtype == CASHSHOP || nd->subtype == SPSHOP || nd->subtype == WOESHOP || nd->subtype == COSTUMESHOP ) && nd->src_id == 0 ) //src check for duplicate shops [Orcao]
	{
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	if( !strcasecmp(w2,"shop") )
		...
		type = SPSHOP;
	else if( !strcasecmp(w2,"xwoeshop") )
		type = WOESHOP; // by isaac
	...
	if( ( type == SPSHOP || type == WOESHOP || type == COSTUMESHOP ) && cashitem >= 0 && itemdb_exists(cashitem) == NULL )
	{
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	// get placement
	if( (type==SHOP || type==CASHSHOP || type==SPSHOP || type == WOESHOP || type == COSTUMESHOP || type==SCRIPT) && strcmp(w1, "-") == 0 )
...
int npc_do_atcmd_event(struct map_session_data* sd, const char* command, const char* message, const char* eventname)
{
	...
	temp[j] = '\0';
	setd_sub(st, NULL, ".@atcmd_parameters$", k++, (void *)temp, NULL);
	setd_sub(st, NULL, ".@atcmd_numparameters", 0, (void *)(intptr_t)k, NULL);
	aFree(temp);
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	else if (!strcmpi(w3,"nodiscount"))//iSaaC
		map[m].flag.nodiscount=state;
	else if (!strcmpi(w3,"nowoeitems"))//iSaaC
		map[m].flag.nowoeitems=state;
	else if (!strcmpi(w3,"mvp"))//iSaaC
...
void npc_parsesrcfile(const char* filepath)
{
	...
	for( p = skip_space(buffer); p && *p ; p = skip_space(p) )
	{
		...
		else if( (!strcasecmp(w2,"shop") || !strcasecmp(w2,"sshop") || !strcasecmp(w2,"cashshop") || !strcasecmp(w2,"xwoeshop") || !strcasecmp(w2,"costumeshop") || (i = 0, sscanf(w2,"cashshop%n",&i), (i > 0 && w2[i] == '('))) && count > 3 )
		{
