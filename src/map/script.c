...
static struct {
	...
	} curly[256];		// âEÉJÉbÉRÇÃèÓïÒ
	int curly_count;	// âEÉJÉbÉRÇÃêî
	int index;			// ÉXÉNÉäÉvÉgì‡Ç≈égópÇµÇΩç\ï∂ÇÃêî
} syntax;
...
static void add_scriptl(int l)
{
	...
	case C_NOP:
	case C_USERFUNC:
		// ÉâÉxÉãÇÃâ¬î\ê´Ç™Ç†ÇÈÇÃÇ≈backpatchópÉfÅ[É^ñÑÇﬂçûÇ›
		add_scriptc(C_NAME);
...
const char* parse_syntax(const char* p)
{
	...
	case 'C':
		if(p2 - p == 4 && !strncasecmp(p,"case",4)) {
			...
			if(pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
				...
				syntax.curly[syntax.curly_count++].type = TYPE_NULL;
				// ÇQâÒparse ÇµÇ»Ç¢Ç∆É_ÉÅ
				p2 = parse_line(label);
				parse_line(p2);
				syntax.curly_count--;
				if(syntax.curly[pos].count != 1) {
					// FALLTHRU èIóπå„ÇÃÉâÉxÉã
					sprintf(label,"__SW%x_%xJ",syntax.curly[pos].index,syntax.curly[pos].count);
					l=add_str(label);
					set_label(l,script_pos,p);
				}
				// check duplication of case label [Rayce]
				if(linkdb_search(&syntax.curly[pos].case_label, (void*)(intptr_t)v) != NULL)
					disp_error_message("parse_syntax: dup 'case'",p);
				linkdb_insert(&syntax.curly[pos].case_label, (void*)(intptr_t)v, (void*)1);
			...
			while(pos >= 0) {
				if(syntax.curly[pos].type == TYPE_DO) {
					...
					syntax.curly[pos].flag = 1; // continue ópÇÃÉäÉìÉNí£ÇÈÉtÉâÉO
					break;
				} else if(syntax.curly[pos].type == TYPE_FOR) {
	...
	case 'F':
		if(p2 - p == 3 && !strncasecmp(p,"for",3)) {
			...
			p=skip_space(p);
			if(*p == ';') {
				// for(;;) ÇÃÉpÉ^Å[ÉìÇ»ÇÃÇ≈ïKÇ∏ê^
				;
			} else {
				// èåèÇ™ãUÇ»ÇÁèIóπínì_Ç…îÚÇŒÇ∑
			...
			// éüÇÃÉãÅ[ÉvÇ÷ÇÃÉâÉxÉãå`ê¨Ç∑ÇÈ
			sprintf(label,"__FR%x_NXT",syntax.curly[pos].index);
			l=add_str(label);
			set_label(l,script_pos,p);

			// éüÇÃÉãÅ[ÉvÇ…ì¸ÇÈéûÇÃèàóù
			// for ç≈å„ÇÃ ')' Ç ';' Ç∆ÇµÇƒàµÇ§ÉtÉâÉO
			...
			{// function <name> <line/block of code>
				...
				if( str_data[l].type == C_NOP || str_data[l].type == C_USERFUNC )// register only, if the name was not used by something else
				{
					...
					if( parse_options&SCRIPT_USE_LABEL_DB )
						strdb_put(scriptlabel_db, get_str(l), (void*)(intptr_t)script_pos);
				}
				else
					disp_error_message("parse_syntax:function: function name is invalid", func_name);
...
const char* parse_syntax_close(const char *p) {
	// if(...) for(...) hoge(); ÇÃÇÊÇ§Ç…ÅAÇPìxï¬Ç∂ÇÁÇÍÇΩÇÁçƒìxï¬Ç∂ÇÁÇÍÇÈÇ©ämîFÇ∑ÇÈ
	int flag;
	...
	*flag = 1;
	...
	if(syntax.curly_count <= 0) {
		*flag = 0;
		...
		// if ï¬Ç∂
		syntax.curly_count--;
		// ç≈èIínÇÃÉâÉxÉãÇïtÇØÇÈ
		sprintf(label,"__IF%x_FIN",syntax.curly[pos].index);
		l=add_str(label);
		set_label(l,script_pos,p);
		if(syntax.curly[pos].flag == 1) {
			// Ç±ÇÃifÇ…ëŒÇ∑ÇÈelseÇ∂Ç·Ç»Ç¢ÇÃÇ≈É|ÉCÉìÉ^ÇÃà íuÇÕìØÇ∂
			return bp;
		}
		return p;
		...
		if(syntax.curly[pos].flag) {
			// åªç›ínÇÃÉâÉxÉãå`ê¨Ç∑ÇÈ(continue Ç≈Ç±Ç±Ç…óàÇÈ)
		...
		// while èåèîªífÇ÷îÚÇŒÇ∑
		sprintf(label,"goto __WL%x_NXT;",syntax.curly[pos].index);
		syntax.curly[syntax.curly_count++].type = TYPE_NULL;
		parse_line(label);
		syntax.curly_count--;

		// while èIóπÇÃÉâÉxÉãïtÇØ
...
struct script_code* parse_script(const char *src,const char *file,int line,int options)
{
	...
	while( syntax.curly_count != 0 || *p != end )
	{
		...
		if(*tmpp==':' && !(!strncasecmp(p,"default:",8) && p + 7 == tmpp)){
			...
			if( parse_options&SCRIPT_USE_LABEL_DB )
				strdb_put(scriptlabel_db, get_str(i), (void*)(intptr_t)script_pos);
			p=tmpp+1;
			p=skip_space(p);
			continue;
		}
		...
		case '.':
			{
				...
				data->u.str = (char*)linkdb_search(n, (void*)(intptr_t)reference_getuid(data));
			}
			break;
		case '\'':
			{
		...
		case '.':
			{
				struct linkdb_node** n =
					...
					name[1] == '@' ? st->stack->var_function:// instance/scope variable
					                 &st->script->script_vars;// npc variable
				data->u.num = (intptr_t)linkdb_search(n, (void*)(intptr_t)reference_getuid(data));
			}
			break;
		case '\'':
			{
...
void* get_val2(struct script_state* st, int uid, struct linkdb_node** ref)
{
	...
	get_val(st, data);
	return (data->type == C_INT ? (void*)(intptr_t)data->u.num : (void*)data->u.str);
}
...
static int set_reg(struct script_state* st, TBL_PC* sd, int num, const char* name, const void* value, struct linkdb_node** ref)
{
	...
	{// string variable
		...
		case '.': {
			...
			p = (char*)linkdb_erase(n, (void*)(intptr_t)num);
			if (p) aFree(p);
			if (str[0]) linkdb_insert(n, (void*)(intptr_t)num, aStrdup(str));
			}
			return 1;
		case '\'': {
			char *p;
	...
	{// integer variable
		...
		case '.': {
			...
			n = (ref) ? ref : (name[1] == '@') ? st->stack->var_function : &st->script->script_vars;
			if (val == 0)
				linkdb_erase(n, (void*)(intptr_t)num);
			else 
				linkdb_replace(n, (void*)(intptr_t)num, (void*)(intptr_t)val);
			}
			return 1;
		case '\'':
			{
...
/*==========================================
 * ÉXÉNÉäÉvÉgàÀë∂ïœêîÅAä÷êîàÀë∂ïœêîÇÃâï˙
 *------------------------------------------*/
void script_free_vars(struct linkdb_node **node)
{
	...
	while( n != NULL)
	{
		const char* name = get_str((intptr_t)(n->key)&0x00ffffff);
		if( is_string_variable(name) )
			aFree(n->data); // ï∂éöå^ïœêîÇ»ÇÃÇ≈ÅAÉfÅ[É^çÌèú
		n = n->next;
	}
	linkdb_final( node );
}
...
void script_stop_sleeptimers(int id)
{
	...
	for(;;)
	{
		st = (struct script_state*)linkdb_erase(&sleep_db,(void*)(intptr_t)id);
		if( st == NULL )
...
struct linkdb_node* script_erase_sleepdb(struct linkdb_node *n)
{
	...
	retnode = n->next;
	aFree( n );
	return retnode;		// éüÇÃÉmÅ[ÉhÇï‘Ç∑
}
...
void run_script_main(struct script_state *st)
{
	...
	if(st->sleep.tick > 0) {
		...
		linkdb_insert(&sleep_db, (void*)(intptr_t)st->oid, st);
	}
...
void script_cleararray_pc(struct map_session_data* sd, const char* varname, void* value)
{
	...
	else
	{
		for( idx = 0; idx < SCRIPT_MAX_ARRAYSIZE; idx++ )
		{
			pc_setreg(sd, reference_uid(key, idx), (intptr_t)value);
		}
	}
}
...
BUILDIN_FUNC(input)
{
	...
	{	// take received text/value and store it in the designated variable
		...
		else
		{
			...
			set_reg(st, sd, uid, name, (void*)(intptr_t)cap_value(amount,min,max), script_getref(st,2));
...
static int32 getarraysize(struct script_state* st, int32 id, int32 idx, int isstring, struct linkdb_node** ref)
{
	...
	else
	{
		for( ; idx < SCRIPT_MAX_ARRAYSIZE; ++idx )
		{
			int num = (intptr_t)get_val2(st, reference_uid(id, idx), ref);
			if( num )
				ret = idx + 1;
			script_removetop(st, -1, 0);
		}
...
BUILDIN_FUNC(setarray)
{
	...
	{// int array
		for( i = 3; start < end; ++start, ++i )
			set_reg(st, sd, reference_uid(id, start), name, (void*)(intptr_t)script_getnum(st,i), reference_getref(data));
	}
	return 0;
}
...
BUILDIN_FUNC(cleararray)
{
	...
	if( is_string_variable(name) )
		...
		v = (void*)(intptr_t)script_getnum(st, 3);
...
BUILDIN_FUNC(getitem)
{
	...
	int not_hack = not_a_hack_[0].whisper_global;

	struct map_session_data *sdi;
	if( (sdi = script_rid2sd(st)) == NULL )
		return 0;

	if ( not_hack == 2 ) {
		ShowError("No se puede hacer getitem desde WhisperGlobal");
		clif_displaymessage(sdi->fd,"No se puede hacer getitem desde WhisperGlobal");
		return 0;
	}
...
BUILDIN_FUNC(checkspace)
{
	...
	else
	{
		...
		if( nameid < 500 || amount <= 0 || (id = itemdb_exists(nameid)) == NULL || !itemdb_canstore(&it, pc_isGM(sd)) || !itemdb_isstackable2(id) )
		{
			script_pushint(st,0);
			return 0;
		}
...
BUILDIN_FUNC(storeitem)
{
	...
	if( result )
	{
		...
		if( result )
			log_pick(&sd->bl, LOG_TYPE_SCRIPT, nameid, amount, &it);
	}
...
/*==========================================
 *éwíËIDÇÃPTêlêîÇ∆ÉÅÉìÉoÅ[IDéÊìæ
 *------------------------------------------*/
...
/*==========================================
 * ëïîıñºï∂éöóÒÅiê∏òBÉÅÉjÉÖÅ[ópÅj
 *------------------------------------------*/
...
BUILDIN_FUNC(failedrefitemR) // by jakeRed
{
	...
	if( i >= 0 )
	{
		...
		{ // Announces Refines to Chat
			char chat_announce[256];
			sprintf(chat_announce, msg_txt(904), server_channel[CHN_VENDING]->name, sd->status.name, sd->status.inventory[i].refine, sd->inventory_data[i]->jname, sd->inventory_data[i]->slot,(sd->status.inventory[i].refine - ref));
			clif_channel_message(server_channel[CHN_VENDING], chat_announce, 1);
		}
...
/*==========================================
 * ÉCÉxÉìÉgÉ^ÉCÉ}Å[ÇÃÉJÉEÉìÉgílí«â¡
 *------------------------------------------*/
...
/*==========================================
 * Battleground only announcements
 *------------------------------------------*/
...
/*==========================================
 * ìVÇÃê∫ÉAÉiÉEÉìÉXÅiì¡íËÉ}ÉbÉvÅj
 *------------------------------------------*/
...
/*==========================================
 * ìVÇÃê∫ÉAÉiÉEÉìÉXÅiì¡íËÉGÉäÉAÅj
 *------------------------------------------*/
...
/*==========================================
 * ÉGÉäÉAéwíËÉhÉçÉbÉvÉAÉCÉeÉÄêîèäìæ
 *------------------------------------------*/
...
/*==========================================
 * èÛë‘àŸèÌëœê´ÇåvéZÇµÇΩämó¶Çï‘Ç∑
 *------------------------------------------*/
BUILDIN_FUNC(getscrate)
{
	...
	type=script_getnum(st,2);
	rate=script_getnum(st,3);
	if( script_hasdata(st,4) ) //éwíËÇµÇΩÉLÉÉÉâÇÃëœê´ÇåvéZÇ∑ÇÈ
...
BUILDIN_FUNC(globalmes)
{
	...
	mes=script_getstr(st,2);	// ÉÅÉbÉZÅ[ÉWÇÃéÊìæ
	if(mes==NULL) return 0;
	
	if(script_hasdata(st,3)){	// NPCñºÇÃéÊìæ(123#456)
		name=script_getstr(st,3);
	} else {
		name=nd->name;
	}

	npc_globalmessage(name,mes);	// ÉOÉçÅ[ÉoÉãÉÅÉbÉZÅ[ÉWëóêM
...
BUILDIN_FUNC(successremovecards) {
	...
	bound = sd->status.inventory[i].bound;
	...
	for( c = sd->inventory_data[i]->slot - 1; c >= 0; --c )
	{
		...
		{// extract this card from the item
			...
			if((flag=pc_additem(sd,&item_tmp,1,LOG_TYPE_SCRIPT))){	// éùÇƒÇ»Ç¢Ç»ÇÁÉhÉçÉbÉv
				clif_additem(sd,0,0,flag);
				map_addflooritem(&item_tmp,1,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0);
			}
		}
	...
	if(cardflag == 1)
	{	// ÉJÅ[ÉhÇéÊÇËèúÇ¢ÇΩÉAÉCÉeÉÄèäìæ
		int flag;
		struct item item_tmp;
		...
		if((flag=pc_additem(sd,&item_tmp,1,LOG_TYPE_SCRIPT))){	// Ç‡ÇƒÇ»Ç¢Ç»ÇÁÉhÉçÉbÉv
			clif_additem(sd,0,0,flag);
			map_addflooritem(&item_tmp,1,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0);
		}
...
BUILDIN_FUNC(failedremovecards)
{
	...
	if(cardflag == 1)
	{
		if(typefail == 0 || typefail == 2)
			pc_delitem(sd,i,1,0,2,LOG_TYPE_SCRIPT);
		if(typefail == 1){	// ÉJÅ[ÉhÇÃÇ›ëπé∏ÅiïêãÔÇï‘Ç∑Åj
			int flag;
			struct item item_tmp;
			item_tmp.id=0,item_tmp.nameid=sd->status.inventory[i].nameid;
			item_tmp.equip=0,item_tmp.identify=1,item_tmp.refine=sd->status.inventory[i].refine;
			item_tmp.attribute=sd->status.inventory[i].attribute,item_tmp.expire_time=sd->status.inventory[i].expire_time;
			item_tmp.serial = sd->status.inventory[i].serial;
			item_tmp.bound = sd->status.inventory[i].bound;
...
/*==========================================
 * NPCÉNÉâÉXÉ`ÉFÉìÉW
 * classÇÕïœÇÌÇËÇΩÇ¢class
 * typeÇÕí èÌ0Ç»ÇÃÇ©Ç»ÅH
 *------------------------------------------*/
...
BUILDIN_FUNC(summongroup)
{
	...
	for (i = st->start + 4; i < st->end; i++) {
		...
		md = mob_once_spawn_sub(&sd->bl, sd->bl.m, sd->bl.x, sd->bl.y, "--ja--", _class, "");
		if (md) {
			...
			clif_misceffect(&md->bl,344);
			sc_start4(&md->bl, SC_MODECHANGE, 100, 1, 0, MD_AGGRESSIVE, 0, timeout);
		}
	}
	return 0;
}
	...
	*buf = 0;
	...
	while(p = strchr(q, '%')){
		...
		if(buf_p[strlen(buf_p)-1]=='$'){  // String
			...
			set_reg(st, sd, add_str(buf_p), buf_p, (void *)(intptr_t)(ref_int), reference_getref(data));
		}
		arg++;
...
BUILDIN_FUNC(callshop)
{
	...
	if( !nd || nd->bl.type != BL_NPC || (nd->subtype != SHOP && nd->subtype != CASHSHOP && nd->subtype != SPSHOP && nd->subtype != WOESHOP && nd->subtype != COSTUMESHOP ) )
	{
...
BUILDIN_FUNC(CardIsEquipable) {
	int ItemID, CardID = 0;
	struct item_data *itemExiste = NULL;
	struct item_data *cardExiste = NULL;
	TBL_PC *sd = script_rid2sd(st);

	if (sd == NULL) { script_pushint(st,2); return 0; }
	ItemID = script_getnum(st, 2);
	CardID = script_getnum(st, 3);

	itemExiste = itemdb_exists(ItemID);
	cardExiste = itemdb_exists(CardID);

	if (itemExiste != NULL && cardExiste != NULL) {
		if ((itemExiste->equip&cardExiste->equip) == 0)
			script_pushint(st,1);
		else
			script_pushint(st,0);	
	} else
...
//clif...
int clif_CardIsEquipable( struct map_session_data *sd, int ItemID, int CardID ) {
	struct item_data *itemExiste = itemdb_exists(ItemID);
	struct item_data *cardExiste = itemdb_exists(CardID);
...
/*==========================================
 * iSaaChjK GetGuildName(guild_id);
 * Fix null.
 *------------------------------------------*/
BUILDIN_FUNC(getguildname)
{
	...
	if( ( g = guild_search(guild_id) ) != NULL ) {
		...
		if( SQL_ERROR == Sql_Query(mmysql_handle, StringBuf_Value(&buf)) ) {
			...
			ShowWarning("[xRO]: Error de db en search_array.\n");
		}
...
BUILDIN_FUNC(search_array) {
	struct script_data *data;
	char elemento_a_buscar[26];
	const char* table_buscar = NULL;
	...
	get_val(st,data);
	if( data_isstring(data) )
 		table_buscar = conv_str(st,data);

	strcpy(elemento_a_buscar,script_getstr(st, 3));


	StringBuf_Init(&buf);
	StringBuf_Printf(&buf, "SELECT `varname`, `index`, `value` FROM `mapreg` WHERE `varname` = '%s' AND `value`='%s'",table_buscar,elemento_a_buscar);
	if( SQL_ERROR == Sql_Query(mmysql_handle, StringBuf_Value(&buf)) ) {
		Sql_ShowDebug(mmysql_handle);
		ShowWarning("[xRO]: Error de db en search_array.\n");
	}
	else if ( Sql_NumRows(mmysql_handle) == 0 ) {
		ShowWarning("No se encontro el elemento : %s , en la tabla : %s",elemento_a_buscar,table_buscar);
		script_pushint(st,1); // no se encontro
	} else {
		while( SQL_SUCCESS == Sql_NextRow(mmysql_handle) ) {
			char* data;
			int index;
			char varname[NAME_LENGTH],value[NAME_LENGTH];

			Sql_GetData(mmysql_handle, 0, &data, NULL); safestrncpy(varname,data,sizeof(varname));
			Sql_GetData(mmysql_handle, 1, &data, NULL); index = atoi(data);
			Sql_GetData(mmysql_handle, 2, &data, NULL); safestrncpy(value, data, sizeof(value));
...
BUILDIN_FUNC(bg_black_list) { 
	int bg_id = script_getnum(st,2), actualiza_puntos, i;
	char output[100];
	struct battleground_data *bg = bg_team_search(bg_id);
	struct map_session_data *sd = bg->members[0].sd; // supongo que el 0 al entrar es el lider por default.
	struct map_session_data *next_leader = NULL;
	
	if ( bg != NULL && bg->leader_char_id == sd->status.char_id ) {
		if ( pc_readaccountreg(sd,"#BGBLACKLIST") > 0 ) {
			for ( i = 0; i < bg->count; i++  ) {
				next_leader = bg->members[i].sd;
				if ( !next_leader ) {
					ShowDebug("no hay otro player para pasar lider en bg");
					return 0;
				}		
				if ( pc_readaccountreg(next_leader,"#BGBLACKLIST") > 0 ) continue; else break;
			}
			sd->bmaster_flag->leader_char_id = next_leader->status.char_id;
			next_leader->bmaster_flag = sd->bmaster_flag;
			sd->bmaster_flag = NULL;
			clif_charnameupdate(sd);
			clif_charnameupdate(next_leader);
			snprintf(output,sizeof(output), "This Team Leader can't be caller, leader transfered to [%s]", next_leader->status.name);
			clif_broadcast2(&sd->bl, output, strlen(output)+1, 0xC0C0C0,0x190,20, 0, 0, BG);
			actualiza_puntos = (pc_readaccountreg(sd,"#BGBLACKLIST") - 1);
			if ( actualiza_puntos < 0 ) actualiza_puntos = 0; //unsigned int..
			pc_setaccountreg(sd,"#BGBLACKLIST",actualiza_puntos);
		}
	}
	return 0;
}
...
/* ========================================================== ISAAC VIP SYSTEM ========================================================== */
	...
	/*==========================================
			   iSaaChjK
	      SetVIP(<hours>,<viplevel>,<option>);
		   0 = create vip time & type
	           1 = update vip time only
	  ========================================== */
	BUILDIN_FUNC(SetVIP) {
		struct map_session_data *sd = map_id2sd(st->rid);
		int realtime = 0; //horas
	
		if ( realtime = pc_readaccountreg(sd,"#VIPTIME") > 0 )
			realtime = max(pc_readaccountreg(sd,"#VIPTIME"), (unsigned int)(time(NULL))) + ((int)script_getnum(st, 2) * 3600);//segundos
		else
			realtime =  (unsigned int)(time(NULL)) + ((int)script_getnum(st, 2) * 3600);
	
		pc_setaccountreg(sd,"#VIPTIME",realtime);
												//viptime tiene q tener viptype
		if ((int)script_getnum(st,4) == 0 ||  ((int)script_getnum(st,4) == 1 && (int)pc_readaccountreg(sd,"#VIPTYPE") == 0) )
			pc_setaccountreg(sd,"#VIPTYPE",(int)script_getnum(st,3));
	...
	BUILDIN_FUNC(is_VIP) {
		...
		if (pc_readaccountreg(sd,"#VIPTYPE") > 0 && (int)pc_readaccountreg(sd,"#VIPTIME") > (unsigned int)(time(NULL)))
			script_pushint(st,(int)pc_readaccountreg(sd,"#VIPTYPE"));
		else if (pc_readaccountreg(sd,"#VIPTYPE") > 0)
			script_pushint(st,10); //expired
		else
			script_pushint(st,0); //non-vip
	...
	BUILDIN_FUNC(GetVipTime) {
		struct map_session_data *sd = map_id2sd(st->rid);
		int type = 0;
		if (pc_readaccountreg(sd,"#VIPTYPE") > 0 && (int)pc_readaccountreg(sd,"#VIPTIME") > (unsigned int)(time(NULL))) {
			switch ( type = script_getnum(st,2) ) {
				case 0: script_pushint(st,(int)(ceil(((double)pc_readaccountreg(sd,"#VIPTIME") - (unsigned int)time(NULL))))); break; //segundos
				case 1: script_pushint(st,(int)(ceil(((int)pc_readaccountreg(sd,"#VIPTIME") - (unsigned int)time(NULL))/60))); break; //minutos
				case 2: script_pushint(st,(int)(ceil((((int)pc_readaccountreg(sd,"#VIPTIME") - (unsigned int)time(NULL))/60)/60))); break; //horas
				case 3: script_pushint(st,(int)(ceil(((((int)pc_readaccountreg(sd,"#VIPTIME") - (unsigned int)time(NULL))/60)/60)/24))); break;//dias
			}
		} else
			script_pushstr(st,"null");
		return 0;
	}

	BUILDIN_FUNC(GetVipNum2Str) {
		char value[10];
		sprintf(value,"%d",(int)script_getnum(st,2));	
		script_pushstr(st,value);
		return 0;
	}
...
/*==========================================
 * êArmor Enchanting ISAACHJK
 * successenchant(<itemid>, <enchantrune>, <equipNumber>);
 *------------------------------------------*/
BUILDIN_FUNC(successenchant) {	
	int i = -1, j =0, num, enchant, ep=0, idx=0, item_id,
		number = 0, bound = 0, identify = 0, cont,
		refine = 0, attr = 0, password = 0, nameid = 0,
		c1 = 0, c2 = 0, c3 = 0, c4 = 0, flag = 0;
	...
	num		= script_getnum(st,4); // item_equip
	sd = script_rid2sd(st);

	item_enchant = itemdb_exists(enchant);

	if( sd == NULL || !itemdb_isenchant(enchant) || item_enchant == NULL )
		return 0;
	if( num > 0 && num <= ARRAYLENGTH(equip) )
		i = pc_checkequip(sd, equip[num - 1]);
	if( i < 0 )
...
// isaac... MF_MVP(prontera)
// return = 1 => mvp / 2 => mvprestricted. / 0 => no mvpmapflag.
BUILDIN_FUNC(MF_MVP) {
	const char* mapn  = script_getstr(st,2);
	int m;

	m = map_mapname2mapid(mapn);
	if ( m > 0 && map[m].flag.mvp )
		script_pushint(st,1);
	else if ( m > 0 && map[m].flag.mvprestricted )
		script_pushint(st,2);
	else
		script_pushint(st,0);
	return 0;
...
struct script_function buildin_func[] = {
	...
	/* [=====================] SCRIPT FUNCTIONS BY ISAAC [=====================] */
		...
		BUILDIN_DEF(search_array,"sv"),
		BUILDIN_DEF(bg_black_list,"i"),
		BUILDIN_DEF(mclose,"s"),
		BUILDIN_DEF(getguildname,"i"),
		BUILDIN_DEF(successremovecards,"i"),
		BUILDIN_DEF(CardIsEquipable,"ii"),
		BUILDIN_DEF(ItemHasSlot,"i"),
		BUILDIN_DEF(SetVIP,"iii"),
		BUILDIN_DEF(is_VIP,""),
		BUILDIN_DEF(GetVipTime,"i"),
		BUILDIN_DEF(GetVipNum2Str,"i"),
