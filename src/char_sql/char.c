...
{	//Insert the barebones to then update the rest.
	...
	if (
		...
		(p->rename != cp->rename) || (p->robe != cp->robe) || (p->playtime != cp->playtime)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`option`='%d',`party_id`='%d',`guild_id`='%d',`pet_id`='%d',`homun_id`='%d',`faction_id`='%d',"
			"`weapon`='%d',`shield`='%d',`head_top`='%d',`head_mid`='%d',`head_bottom`='%d',"
			"`last_map`='%s',`last_x`='%d',`last_y`='%d',`save_map`='%s',`save_x`='%d',`save_y`='%d', `rename`='%d',"
			"`delete_date`='%lu',`robe`='%d',`playtime`='%u'"
			" WHERE  `account_id`='%d' AND `char_id` = '%d'",
			char_db, p->base_level, p->job_level,
			p->base_exp, p->job_exp, p->zeny,
			p->max_hp, p->hp, p->max_sp, p->sp, p->status_point, p->skill_point,
			p->str, p->agi, p->vit, p->int_, p->dex, p->luk,
			p->option, p->party_id, p->guild_id, p->pet_id, p->hom_id, p->faction_id,
...
int memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
	...
	StringBuf_Init(&buf);
	StringBuf_AppendStr(&buf, "SELECT `id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `serial`, `bound`, `favorite`");
	...
	SqlStmt_BindColumn(stmt, 8, SQLDT_UINT,      &item.serial,      0, NULL, NULL);
	...
	while( SQL_SUCCESS == SqlStmt_NextRow(stmt) )
	{
		...
		for( i = 0; i < max; ++i )
		{
			...
			if( items[i].nameid == 0 || flag[i] )
				continue;
			if( items[i].serial && items[i].serial != item.serial )
				continue;
			...
			&&  items[i].card[3] == item.card[3]
				...
				{ // Differences founds, updated required
					StringBuf_Clear(&buf);
					StringBuf_Printf(&buf, "UPDATE `%s` SET `amount`='%d', `equip`='%d', `identify`='%d', `refine`='%d',`attribute`='%d', `expire_time`='%u', `serial`='%u', `bound`='%d', `favorite`='%d'",
						tablename, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].serial, items[i].bound, items[i].favorite);
					for( j = 0; j < MAX_SLOTS; ++j )
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `serial`, `bound`, `favorite`", tablename, selectoption);
	for( j = 0; j < MAX_SLOTS; ++j )
		StringBuf_Printf(&buf, ", `card%d`", j);
	...
	for( i = 0; i < max; ++i )
	{
		...
		StringBuf_Printf(&buf, "('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', '%d', '%d'",
			id, items[i].nameid, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].serial, items[i].bound, items[i].favorite);
		for( j = 0; j < MAX_SLOTS; ++j )
...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`save_map`,`save_x`,`save_y`,`partner_id`,`father`,`mother`,`child`,`fame`,`rename`,`delete_date`,`robe`,`playtime`,`faction_id`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 8, SQLDT_UINT,      &tmp_item.serial, 0, NULL, NULL)
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT c.`account_id`, c.`char_id`, c.`name`, c.`class` FROM `%s` c LEFT JOIN `%s` f ON f.`friend_account` = c.`account_id` AND f.`friend_id` = c.`char_id` WHERE f.`char_id`=? LIMIT %d", char_db, friend_db, MAX_FRIENDS)
	||	SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0)
	||	SQL_ERROR == SqlStmt_Execute(stmt)
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 3, SQLDT_INT,    &tmp_friend.class_, 0, NULL, NULL) ) // @sjob [csnv]
		SqlStmt_ShowDebug(stmt);
...
//-----------------------------------
// Save Character Data into SQL File iSaaC Fix
//-----------------------------------
...
{	
	...
	fprintf(fp, "INSERT INTO `char` "
		...
		" VALUES "
		"('%d','%d', '%d', '%s', '%d', '%d', '%d', '%u', '%u', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u');\n",
		cp.char_id,cp.account_id,cp.slot, esc_name, cp.class_, cp.base_level, cp.job_level, cp.base_exp, cp.job_exp, cp.zeny, cp.str, cp.agi, cp.vit, cp.int_, cp.dex, cp.luk, cp.max_hp, cp.hp, cp.max_sp, cp.sp, cp.status_point, cp.skill_point, cp.option, cp.karma, cp.manner, 0, 0, 0, 0, cp.hair, cp.hair_color, cp.clothes_color, cp.weapon, cp.shield, cp.head_top, cp.head_mid, cp.head_bottom, mapindex_id2name(cp.last_point.map), cp.last_point.x, cp.last_point.y, mapindex_id2name(cp.save_point.map), cp.save_point.x, cp.save_point.y, 0, 0, 0, 0, 0, cp.fame, cp.playtime);
	...
	{ // Backup Account Information
		...
		while( i < MAX_STORAGE && cp.storage.items[i].nameid > 0 )
		{
			...
			fprintf(fp, "INSERT INTO `storage` "
				"(`account_id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `card0`, `card1`, `card2`, `card3`, `expire_time`, `serial`, `bound`)"
				" VALUES "
				"('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', '%d');\n",
				cp.account_id,i_data->nameid, i_data->amount, i_data->equip, i_data->identify, i_data->refine, i_data->attribute, i_data->card[0], i_data->card[1], i_data->card[2], i_data->card[3], i_data->expire_time, i_data->serial, i_data->bound);
			i++;
		}
	...
	while( i < MAX_INVENTORY && cp.inventory[i].nameid > 0 )
	{
		...
		fprintf(fp, "INSERT INTO `inventory` "
			"(`char_id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `card0`, `card1`, `card2`, `card3`, `expire_time`, `serial`, `bound`)"
			" VALUES "
			"('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', '%d');\n",
			cp.char_id,i_data->nameid, i_data->amount, i_data->equip, i_data->identify, i_data->refine, i_data->attribute, i_data->card[0], i_data->card[1], i_data->card[2], i_data->card[3], i_data->expire_time, i_data->serial, i_data->bound);
		i++;
	}
	...
	while( SQL_SUCCESS == Sql_NextRow(sql_handle) )
	{
		...
		fprintf(fp, "INSERT INTO `global_reg_value` (`char_id`, `str`, `value`, `type`, `account_id`) VALUES ('%d', '%s', '%s', '3', '0');\n",
			cp.char_id,esc_str, esc_value);
	}
