...
void initChangeTables(void)
{
	...
	set_sc( AM_CP_WEAPON         , SC_CP_WEAPON       , SI_BLANK       , SCB_NONE );
	set_sc( AM_CP_SHIELD         , SC_CP_SHIELD       , SI_BLANK       , SCB_NONE );
	set_sc( AM_CP_ARMOR          , SC_CP_ARMOR        , SI_BLANK        , SCB_NONE );
	...
	set_sc( SL_SKA               , SC_SKA             , SI_BLANK           , SCB_STR|SCB_DEF|SCB_MDEF|SCB_ASPD );
	...
	//SkillStatusChangeTable[SC_SKE]	= SI_ODINSPOWER;
	...
	// Iconos de estados alterados xRO
	...
	StatusIconChangeTable[SC_BATKFOOD] = SI_BATKFOOD;
	StatusIconChangeTable[SC_INCATKRATE] = SI_INCATKRATE;
	StatusIconChangeTable[SC_INCHIT] = SI_INCHIT;
	StatusIconChangeTable[SC_INCFLEE2] = SI_INCFLEE2;
	StatusIconChangeTable[SC_INCLUK] = SI_INCLUK;
	StatusIconChangeTable[SC_INCMHPRATE] = SI_INCMHPRATE;
	StatusIconChangeTable[SC_INCMSPRATE] = SI_INCMSPRATE;
	StatusIconChangeTable[SC_DEVOTION] = SI_DEVOTION;
	StatusIconChangeTable[SC_ARMOR_ELEMENT] = SI_ARMOR_PROPERTY;
	StatusIconChangeTable[SC_SERVICE4U] = SI_SERVICEFORYOU;
	StatusIconChangeTable[SC_DONTFORGETME] = SI_DONTFORGETME;
	StatusIconChangeTable[SC_ASSNCROS] = SI_ASSASSINCROSS;
...
//If flag&64, reflected damage, should not change target.
...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	if( hp && !(flag&1) ) {
		if( sc ) {
			...
			if( src && !((battle_config.reflect_damage_fix&2) && (flag&64)) && skill != PA_PRESSURE )
			{ // Check for Target change defenses
				...
				if( (sce = sc->data[SC_DEVOTION]) )
				{
					...
					{
						clif_damage(d_bl, d_bl, gettick(), 0, 0, devo_hp, 0, 0, 0);
						status_fix_damage(NULL, d_bl, devo_hp, 0, 0);
						return 0;
					}
...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	status->size = fd ? fd->size : 1; // Default Size is Medium
	...
	if( status->size < 2 && battle_config.character_size && pc_isriding(sd) )
	{
		...
		{
			if( battle_config.character_size&2 ) status->size++;
		}
		else if( battle_config.character_size&1 ) status->size++;
	}
...
// ----- MISC CALCULATIONS -----
	...
	//ISAAC BUFF WHITSHMITH
	if((skill=pc_checkskill(sd,BS_SKINTEMPER))>0) 
	{
		sd->subele[ELE_NEUTRAL] += skill;
		sd->subele[ELE_FIRE] += skill*4;
		sd->subele[ELE_EARTH] += skill*4;
		sd->subele[ELE_WIND] += skill*4;
		sd->subele[ELE_WATER] += skill*4;
	}
	if((skill=pc_checkskill(sd,SA_DRAGONOLOGY))>0 ){
		skill = skill*4;
...
static unsigned short status_calc_str(struct block_list *bl, struct status_change *sc, int str)
{
	...
	if(sc->data[SC_SPIRIT] && sc->data[SC_SPIRIT]->val2 == SL_HIGH && str < 50)
		str = 50;
	if(sc->data[SC_SKE])
		str += str * sc->data[SC_SKE]->val1/100; // 30% demihumanos, ISAAC.
...
static unsigned short status_calc_batk(struct block_list *bl, struct status_change *sc, int batk)
{
	...
	if(sc->data[SC_SKE]) //ISAAC
		batk +=  batk * sc->data[SC_SKE]->val1/100; // 30% demihumanos, ISAAC.
...
static unsigned short status_calc_watk(struct block_list *bl, struct status_change *sc, int watk)
{
	...
	if(sc->data[SC_SKE])
		watk += watk * sc->data[SC_SKE]->val1/100; // 30% demihumanos, ISAAC.
...
static unsigned short status_calc_speed(struct block_list *bl, struct status_change *sc, int speed)
{
	...
	else
	{
		...
		//GetMoveHasteValue2()
		{
			...
			if( sc->data[SC_FUSION] || sc->data[SC_ALL_RIDING] )
				val = 25;
			else
	...
	if (!(flag&8)) {
		...
		if(sd && SC_COMMON_MIN <= type && type <= SC_COMMON_MAX)
		{
			...
			if( sd->sc.data[SC_COMMONSC_RESIST] )
				...
				rate -= 5000 * sd->sc.data[SC_COMMONSC_RESIST]->val1 / 100; // 10% siempre [csnv]
		}
	}
	if (!(rand()%10000 < rate))
		return 0;
	...
	switch( type )
	{
		...
		case SC_CHASEWALK:
			if( sd )
			{
				if( sd->view_aura )
				{
					clif_clearunit_area(bl,CLR_OUTSIGHT);//iSaaC enum FIX
...
int status_change_end(struct block_list* bl, enum sc_type type, int tid)
{
	...
	case SC_HIDING:
		...
		if ( sd && map[sd->bl.m].flag.battleground )
			skill_blockpc_start (sd, MO_EXTREMITYFIST, 1500);
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_AUTOREFRESH:
		if( sd )
		{
			...
			clif_status_change(&sd->bl,StatusIconChangeTable[type],1,duration);
	...
	case SC_BLEEDING:
		if (--(sce->val4) >= 0) {
			...
			map_freeblock_lock();
			status_fix_damage(NULL, bl, sd||hp<status->hp?hp:status->hp-1, 0, 0);
			flag = !sc->data[type];
			map_freeblock_unlock();
			if( !flag ) {
