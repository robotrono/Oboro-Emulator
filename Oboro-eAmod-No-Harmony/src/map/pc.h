...
struct map_session_data {
	...
	struct {
		...
		unsigned short combined_view; // Flag para indicar vista de jobs por party [csnv]
		unsigned short nodelay; // [iSaaC]
		...
		unsigned int restock : 1; //iSaaChjK
		unsigned int afk : 1; //iSaaC
		unsigned int hold : 1; //isaachjk
		unsigned int oboro_showinfo : 1;
		unsigned int vending_cash_tax : 1; //isaac
	} state;
	...
	unsigned int oboro_last_skill_id;
	unsigned int oboro_hitlock;
	unsigned int oboro_hitlock_until;
	time_t oboro_skill_tick;
	int oboro_hackcount;
	int oboro_allowedReduction;
	int oboro_jauge;
	int oboro_infixed;
