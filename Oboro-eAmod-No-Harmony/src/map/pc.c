...
static const unsigned short equip_pos[EQI_MAX]={EQP_ACC_L,EQP_ACC_R,EQP_SHOES,EQP_GARMENT,EQP_HEAD_LOW,EQP_HEAD_MID,EQP_HEAD_TOP,EQP_ARMOR,EQP_HAND_L,EQP_HAND_R,EQP_AMMO,EQP_COS_HEAD_TOP,EQP_COS_HEAD_MID,EQP_COS_HEAD_LOW};
...
void pc_fix_items_sub(struct item *it, struct item_data *inventory_data, struct mail_message *msg)
{
	...
	// Serialize if not
	if( !itemdb_isstackable2(item) && it->serial == 0 )
		it->serial = itemdb_getserial(item);
...
{
	return ( sd->Premium_Tick > (int)time(NULL) );
}
...
void pc_ranking_reset(int type, bool char_server)
{
	...
	for( sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		case 2: // PVP Event Ranking
			Sql_Query(mmysql_handle, "DELETE FROM `global_reg_value` WHERE `str` = 'PlayersKilled' OR `str`='PlayersDie'");
...
int pc_isequip(struct map_session_data *sd,int n)
{
	...
	/*			WOEITEM NO EQUIP MAPFLAG BY ISAAC				*/
	//ShowWarning("Se esta equipando el item_id %d", sd->status.inventory[n].nameid);
	if ( map[sd->bl.m].flag.nowoeitems ) {
		if ( sd->status.inventory[n].expire_time > 0 && sd->status.inventory[n].bound == 5 )
			return 0;
	}
	...
	if ( map[sd->bl.m].flag.gvg_castle || map[sd->bl.m].flag.gvg || map[sd->bl.m].flag.woe_set ) 
	{
		...
		switch(sd->status.inventory[n].nameid)
		{
			...
			case 1281: // G. Katar
			case 1282: // G. Katar 2
				if ( sd->status.inventory[n].refine > 9 )
					return 0;
			break;
		}
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	sd->Premium_Tick = pc_readaccountreg(sd,"#Premium_Tick");
	...
	if( battle_config.channel_system_enable )
	{
		...
		if( pc_isGM(sd) >= battle_config.lowest_gm_level && (sd->channels&8) && server_channel[CHN_GAMEMASTER] )
...
int pc_modifybuyvalue(struct map_session_data *sd,int orig_value)
{
	...
	if ( !map[sd->bl.m].flag.nodiscount && rate1 ) //iSaaC
...
/*==========================================
 * Cash Shop - #CASHPOINTS and #KAFRAPOINTS
 *------------------------------------------*/
...
int pc_delitem(struct map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	...
	if ( log_type != LOG_TYPE_STORAGE )
		ConsumeEvent2(sd,sd->status.inventory[n].nameid);
...
int pc_cart_additem(struct map_session_data *sd,struct item *item_data,int amount,e_log_pick_type log_type)
{
	...
	log_pick(&sd->bl,log_type,item_data->nameid,amount,&sd->status.cart[i]);
...
int pc_cart_delitem(struct map_session_data *sd,int n,int amount,int type,e_log_pick_type log_type)
{
	...
	log_pick(&sd->bl,log_type,sd->status.cart[n].nameid,-amount,&sd->status.cart[n]);
...
int pc_steal_item(struct map_session_data *sd,struct block_list *bl, int lv)
{
	...
	if(md->state.steal_flag == UCHAR_MAX || md->sc.opt1 || md->option.no_expdrop) //already stolen from / status change check
		return 0;
	...
	flag = pc_additem(sd,&tmp_item,1,LOG_TYPE_STEAL);
	...
	log_pick(&md->bl, LOG_TYPE_STEAL, itemid, -1, &tmp_item);
...
struct { 
	...
	{ "novice",		0 },			{ "swordsman",	1 },			{ "mage",		2 },		{ "archer",		3 },		{ "acolyte",	4 },
	{ "merchant",	5 },				{ "thief",		6 },		{ "knight",		7 },		{ "priest",		8 },		{ "priestess",	8 },
	{ "wizard",		9 },			{ "blacksmith",	10 },			{ "hunter",		11 },		{ "assassin",		12 },		{ "crusader",	14 },
	{ "monk",		15 },			{ "sage",		16 },		{ "rogue",		17 },		{ "alchemist",		18 },		{ "bard",	19 },
	{ "dancer",		20 },			{ "super novice",	23 },		{ "supernovice",	23 },		{ "gunslinger",		24 },		{ "gunner",	24 },
	{ "ninja",	25 },				{ "high novice",	4001 },		{ "swordsman high",	4002 },		{ "mage high",		4003 },		{ "archer high",	4004 },
	{ "acolyte high",	4005 },			{ "merchant high",	4006 },		{ "thief high",		4007 },		{ "lord knight",	4008 },		{ "high priest",	4009 },
	{ "high priestess",	4009 },			{ "high wizard",	4010 },		{ "whitesmith",		4011 },		{ "sniper",		4012 },		{ "assassin cross",	4013 },
	{ "paladin",	4015 },				{ "champion",	4016 },			{ "professor",	4017 },			{ "stalker",	4018 },			{ "creator",	4019 },
	{ "clown",		4020 },			{ "gypsy",		4021 },			{ "baby novice",4023 },		{ "baby swordsman",	4024 },		{ "baby mage",		4025 },
	{ "baby archer",	4026 },			{ "baby acolyte",	4027 },		{ "baby merchant",	4028 },		{ "baby thief",		4029 },		{ "baby knight",	4030 },
	{ "baby priest",	4031 },			{ "baby priestess",	4031 },		{ "baby wizard",	4032 },		{ "baby blacksmith",4033 },		{ "baby hunter",	4034 },
	{ "baby assassin",	4035 },			{ "baby crusader",	4037 },		{ "baby monk",		4038 },		{ "baby sage",		4039 },		{ "baby rogue",		4040 },		
	{ "baby alchemist",	4041 },			{ "baby bard",		4042 },		{ "baby dancer",	4043 },		{ "super baby",		4045 },		{ "taekwon",		4046 },
	{ "taekwon boy",	4046 },			{ "taekwon girl",	4046 },		{ "star gladiator",	4047 },		{ "soul linker",	4049 },
	
	{ "rune knight",	4054 },			{ "warlock",		4055 },		{ "ranger",		4056 },		{ "archbishop",		4057 },		{ "mechanic",		4058 },
	{ "guillotinecross",	4059 },			{ "royal guard",	4066 },		{ "sorcerer",		4067 },		{ "minstrel",		4068 },		{ "wanderer",		4069 },
	{ "shura",		4070 },			{ "genetic",		4071 },		{ "shadow chaser",	4072 },
};

int pc_job2id( char jobName[] ) {
	int i;
	int JobReturn = 0;
	for ( i = 0; i < ARRAYLENGTH(jobs); i++ ) {
		if ( strncmpi(jobName, jobs[i].name, 16) == 0 ) {
			JobReturn = jobs[i].id;
			break;
		}
	}
...
/* =========================================
   ...
   ========================================= */
int pc_ShowJobInfo( const int fd ) {
	clif_displaymessage(fd, "Please, enter job ID (usage: @job <job_id>).");
	clif_displaymessage(fd, " ");
	clif_displaymessage(fd, " ================================== NORMAL JOB's ================================== ");
	clif_displaymessage(fd, " ");
	...
	clif_displaymessage(fd, " ");
	clif_displaymessage(fd, " ================================== TRANSD JOB's ================================== ");
	clif_displaymessage(fd, " ");
	clif_displaymessage(fd, "       4001 H.Novice              4008 Lord Knight             4015 Paladin");
	clif_displaymessage(fd, "       4002 H.Swordman            4009 High Priest             4016 Champion");
	clif_displaymessage(fd, "       4003 H.Mage                4010 High Wizard             4017 Professor");
	clif_displaymessage(fd, "       4004 H.Archer              4011 Whitesmith              4018 Stalker");
	clif_displaymessage(fd, "       4005 H.Acolyte             4012 Sniper                  4019 Creator");
	clif_displaymessage(fd, "       4006 H.Merchant            4013 Assassin Cross          4020 Clown");
	clif_displaymessage(fd, "       4007 H.Thief               4014 N/A                     4021 Gypsy");
	clif_displaymessage(fd, " ");
	clif_displaymessage(fd, " ================================== BABY'S JOB's ================================== ");
	clif_displaymessage(fd, " ");
	clif_displaymessage(fd, "       4023 B.Novice        4030 B.Knight       4037 B.Crusader     4044 N/A");
	clif_displaymessage(fd, "       4024 B.Swordsman     4031 B.Priest       4038 B.Monk         4045 Spr.Baby");
	clif_displaymessage(fd, "       4025 B.Mage          4032 B.Wizard       4039 B.Sage         4046 Taek.Kid");
	clif_displaymessage(fd, "       4026 B.Archer        4033 B.Blacksmith   4040 B.Rogue        4047 Taek.Mstr");
	clif_displaymessage(fd, "       4027 B.Acolyte       4034 B.Hunter       4041 B.Alchemist    4048 N/A");
	clif_displaymessage(fd, "       4028 B.Merchant      4035 B.Assassin     4042 B.Bard         4049 S. Linker");
	clif_displaymessage(fd, "       4029 B.Thief         4036 N/A            4043 B.Dancer");
	clif_displaymessage(fd, " ");
	clif_displaymessage(fd, " ================================================================================== ");
	return 0;
}
...
int pc_setpos(struct map_session_data* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	if( map[m].flag.guild_min && pc_isGM(sd) < 60 )
	{ // isaac Guild Min Limit
	...
	/* isaac @load, savepoint, etc.. house fix */
	else if (strncmp(map[m].name, "gr_", 3) == 0 && pc_isGM(sd) < 60 ) {
		sscanf(map[m].name, "%*[^_]_%d", &numCasa);
		...
		if ( !sd->status.guild_id || (
			...
			clif_displaymessage(sd->fd, "[XRO House]: Ya no se te permite acceder a casas de guild, tu nuevo destino ser· prontera...");
			pc_setsavepoint(sd, mapindex_name2id(MAP_PRONTERA), 155, 183);
			pc_setpos(sd, mapindex_name2id(MAP_PRONTERA), 155, 183, CLR_TELEPORT);
			return 1;
		}
	} else if (map[m].party_max && sd->bl.m != m && sd->status.party_id && (p = party_search(sd->status.party_id)) != NULL) { // Party Limit
...
void pc_damage(struct map_session_data *sd,struct block_list *src,unsigned int hp, unsigned int sp)
{
	...
	{
		// [Zephyrus] Actualizacion de Timer de daÒo PVP, en PK Mode
	...
	if ((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE && !sd->state.snovice_dead_flag)
  	{
		...
		if( get_percentage(sd->status.base_exp,next) >= 99 && !map_flag_gvg3(sd->bl.m) )
		{
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos)
{
	...
	if(pos & EQP_HEAD_LOW && pc_checkequip(sd,EQP_COS_HEAD_LOW) == -1) {
	...
	if(pos & EQP_HEAD_TOP && pc_checkequip(sd,EQP_COS_HEAD_TOP) == -1) {
		if(id)
			sd->status.head_top = id->look;
		else
			sd->status.head_top = 0;
	...
	if(pos & EQP_HEAD_MID && pc_checkequip(sd,EQP_COS_HEAD_MID) == -1) {
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	if(sd->status.inventory[n].equip & EQP_COS_HEAD_MID) {
		...
		clif_changelook(&sd->bl,LOOK_HEAD_MID,sd->status.head_mid);
	*/
	if(sd->status.inventory[n].equip & EQP_HEAD_LOW && pc_checkequip(sd,EQP_COS_HEAD_LOW) == -1 ) {
		sd->status.head_bottom = 0;
		clif_changelook(&sd->bl,LOOK_HEAD_BOTTOM,sd->status.head_bottom);
	}
	if(sd->status.inventory[n].equip & EQP_HEAD_TOP && pc_checkequip(sd,EQP_COS_HEAD_TOP) == -1 ) {
		sd->status.head_top = 0;
		clif_changelook(&sd->bl,LOOK_HEAD_TOP,sd->status.head_top);
	}
	if(sd->status.inventory[n].equip & EQP_HEAD_MID && pc_checkequip(sd,EQP_COS_HEAD_MID) == -1 ) {
		sd->status.head_mid = 0;
		clif_changelook(&sd->bl,LOOK_HEAD_MID,sd->status.head_mid);
	}
	...
	for( i = 0; i < MAX_INVENTORY; i++)
	{
		...
		{ // check for forbiden items.
			...
			if( flag && (it->flag.no_equip&flag || !pc_isAllowedCardOn(sd,it->slot,i,flag)) ) {
				pc_unequipitem(sd, i, 2);
				calc_flag = 1;
			} else if ( map[sd->bl.m].flag.nowoeitems ) {
				//mapflag nowoeitems isaac.
				if ( sd->status.inventory[i].expire_time > 0 && sd->status.inventory[i].bound == 5 )
					pc_unequipitem(sd, i, 2);
					calc_flag = 1;
			} else if ( map[sd->bl.m].flag.gvg_castle || map[sd->bl.m].flag.gvg || map[sd->bl.m].flag.woe_set ) {
				/*	ISAAC NO PERMITE GLORIOUS BOW +7.8.9.10 EN MF GVG... */
				...
				switch(sd->status.inventory[i].nameid)
				{
					...
					case 1281: // G. Katar
					case 1282: // G. Katar 2
						if ( sd->status.inventory[i].refine > 9 )
						{
							pc_unequipitem(sd, i, 2);
							calc_flag = 1;	
						}
			...
			} else if( map[sd->bl.m].flag.ancient && (!it->ancient || !pc_isAllowedCardOn_Ancient(sd,it->slot,i))) {
				pc_unequipitem(sd, i, 2);
				calc_flag = 1;
			}
		}
...
void pc_pvpmodeoff(struct map_session_data* sd, short force, short flag)
{
	...
	{ // Manual Change - else it's a off from change map
		clif_clearunit_area(&sd->bl,CLR_OUTSIGHT); // little fix isaac
		clif_refresh(sd);
		map_foreachinrange(clif_insight_tbl2bl,&sd->bl,AREA_SIZE,BL_PC,&sd->bl);
	}
