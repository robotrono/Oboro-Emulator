...
int battle_delay_damage_sub(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	{
		map_freeblock_lock();
		status_damage_(dat->src,target,dat->damage,0,dat->delay,dat->reflected?64:0,dat->skill_id);
...
int battle_delay_damage(unsigned int tick, int amotion, struct block_list *src, struct block_list *target, int attack_type, int skill_id, int skill_lv, int damage, enum damage_lv dmg_lv, int ddelay, bool reflected)
{
	...
	if (!battle_config.delay_battle_damage) {
		map_freeblock_lock();
		status_damage_(src,target,damage,0,ddelay,reflected?64:0,skill_id);
...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv)
{
	...
	if( sc && sc->count )
	{
		...
		if( (sce=sc->data[SC_AUTOGUARD]) && flag&BF_WEAPON && !(skill_get_nk(skill_num)&NK_NO_CARDFIX_ATK) && rand()%100 < sce->val2 )
		{
			...
			if(sc->data[SC_SHRINK] && !map[bl->m].flag.ancient && rand()%100<5*sce->val1)
		...
		if((sce=sc->data[SC_REJECTSWORD]) && flag&BF_WEAPON &&
			...
			status_fix_damage(bl,src,rejected,clif_damage(bl,src,gettick(),0,0,rejected,0,0,0),skill_num);
	...
	if( sd && damage && (battle_config.pk_mode || (s_bl && s_bl->type == BL_PC && (guild_wardamage(sd) || (((TBL_PC *)src)->state.pvpmode && battle_config.pvpmode_gvgreductions)))) )
  	{
...
static struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	if( tstatus->mode&MD_PLANT ||
		(target->type == BL_MOB && ((TBL_MOB*)target)->state.inmunity) )
		flag.infdef = 1;
	...
	{	//There is a total damage value
		if( wd.damage )
		{
			...
			if( map_flag_gvg2(target->m) )
				...
				wd.damage = battle_calc_pvpevent_damage(src,target,wd.damage,wd.div_,skill_num,skill_lv,wd.flag);
			else if( map[target->m].flag.fvf )
				wd.damage = battle_calc_faction_damage(src,target,wd.damage,wd.div_,skill_num,skill_lv,wd.flag);
		}
		if( wd.damage2 )
		{
			...
			if( map_flag_gvg2(target->m) )
				...
				wd.damage2 = battle_calc_bg_damage(src,target,wd.damage2,wd.div_,skill_num,skill_lv,wd.flag);
			else if( map_pvpevent(target->m) )
				wd.damage2 = battle_calc_pvpevent_damage(src,target,wd.damage2,wd.div_,skill_num,skill_lv,wd.flag);
			else if( map[target->m].flag.fvf )
				wd.damage2 = battle_calc_faction_damage(src,target,wd.damage2,wd.div_,skill_num,skill_lv,wd.flag);
		}
	}
...
struct Damage battle_calc_magic_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	if( tstatus->mode&MD_PLANT || (target->type == BL_MOB && ((TBL_MOB*)target)->state.inmunity) )
		flag.infdef = 1;
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	if(md.damage < 0)
		md.damage = 0;
	else if( md.damage && (tstatus->mode&MD_PLANT || (target->type == BL_MOB && ((TBL_MOB*)target)->state.inmunity)) )
		md.damage = 1;
	...
	if( map_flag_gvg2(target->m) )
		...
		md.damage=battle_calc_pvpevent_damage(src,target,md.damage,md.div_,skill_num,skill_lv,md.flag);
	if( map[target->m].flag.fvf )
		md.damage=battle_calc_faction_damage(src,target,md.damage,md.div_,skill_num,skill_lv,md.flag);
...
struct Damage battle_calc_attack(int attack_type,struct block_list *bl,struct block_list *target,int skill_num,int skill_lv,int count)
{
	...
	{
		d.dmg_lv = ATK_DEF;
		sc_start(target, SC_CUTANIMATION, 100, 0, 700); // ADES hit simulation [csnv]
	}
	return d;
}
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, unsigned int tick, int flag)
{
	...
	battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv, wd.dmotion, false);
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Check rivalry settings.
		...
		if( state&BCT_ENEMY )
		{
			...
			if( map[m].flag.battleground && sbg_id && sbg_id == tbg_id )
				state &= ~BCT_ENEMY;
			if( map[m].flag.fvf && (faction_get_id(s_bl) == faction_get_id(t_bl) || ((id = faction_get_alliance(s_bl)) > 0 && id == faction_get_alliance(t_bl))) )
				state &= ~BCT_ENEMY;
		}
...
// Custom TerraRO Settings
	...
	{ "vending_cash_tax",                   &battle_config.vending_cash_tax,              	0,      0,      INT_MAX,        },//isaac
	...
	{ "oboro_incr",			&battle_config.oboro_incr,				1,	 0,	INT_MAX,        },
	{ "oboro_hitlock_incr",		&battle_config.oboro_hitlock_incr,			1,       0,	INT_MAX,        },
	{ "oboro_max_hitlock_count",	&battle_config.oboro_max_hitlock_count,			3,	 0,	INT_MAX,        },

	{ "oboro_reduce_hitlock_on_correct",&battle_config.oboro_reduce_hitlock_on_correct,	0,	 0,	INT_MAX,        },
	{ "oboro_reduce_hitlock_on_hitlock",&battle_config.oboro_reduce_hitlock_on_hitlock,	1,	 0,	INT_MAX,        },
	{ "oboro_enable",		&battle_config.oboro_enable,				1,	 0,	INT_MAX,        },
	{ "oboro_reduce_hackcount_on_correct",&battle_config.oboro_reduce_hackcount_on_correct, 1,	 0,	INT_MAX,        },
	{ "oboro_reduce_hackcount_on_hitlock",&battle_config.oboro_reduce_hackcount_on_hitlock, 1,	 0,	INT_MAX,        },
	{ "oboro_buffer_time",		&battle_config.oboro_buffer_time, 			50,	 0,	INT_MAX,        },
	{ "oboro_buffer_size",		&battle_config.oboro_buffer_size,			4,	 0,	INT_MAX,        },
	{ "oboro_max_infixed",		&battle_config.oboro_max_infixed,			10,	 0,	INT_MAX,        },
