...
static int lang_db_clear(DBKey key, void *data, va_list ap)
{
	int i, j;
	struct lang_data *ld = (struct lang_data*)data;
...
int faction_get_alliance(struct block_list* bl)
{
	...
	if( bl && (id = faction_get_id(bl)) > 0 && (fd = faction_search(id)) != NULL )
		return fd->alliance_flag;

	return 0;
}

static int faction_db_clear(DBKey key, void *data, va_list ap)
{
	struct faction_data *fd = (struct faction_data*)data;
	if( fd->script )
	{
