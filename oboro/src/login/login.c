...
void login_auth_ok(struct login_session_data* sd)
{
	...
	/* NUEVO EXE PROTECCION // ISAAC 33C083F906740D -> 33C083F906EB0D*/
	if ( traspasa_hash == 0 || ( traspasa_hash == 1 && sd->level == 0 ) ) {
		if ( client_hash_active == false )
			ShowXRO("Custom hash desactivado, configurelo en login_athena.conf .\n");
		else if( memcmp(client_hash, session[fd]->hash, 16) != 0 ) {
			sprintf(output,"%s",client_hash);
			ShowXRO(output);
			ShowXRO("Estado del hash: %s, Traspasa hash: %d, Log hash: %d. \n", (client_hash_active ? "activado" : "desactivado"), traspasa_hash, log_hash);
			hash = session[fd]->hash;
			sprintf(output,"%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X del usuario %s.\n",
				hash[0], hash[1], hash[2], hash[3], hash[4], hash[5], hash[6], hash[7], hash[8], 
				hash[9], hash[10], hash[11], hash[12], hash[13], hash[14], hash[15], sd->userid
			);
			ShowXRO("hash incompatible recibido %s de la cuenta %d, exe ilegal. \n", output, sd->account_id);
			login_auth_failed(sd,101);
			return;
		}	
	}
...
//-----------------------------------
// Read Hash config
// iSaaC based on other systems.
//-----------------------------------
int hash_config_read(const char* cfgName)
{
	char line[1024], w1[1024], w2[1024];
	FILE* fp = fopen(cfgName, "r");
	if (fp == NULL) {
	...
	ShowXRO("Leyendo la configuracion del hash check, conf/security_hash\n");
	while(fgets(line, sizeof(line), fp))
	{
		if (line[0] == '/' && line[1] == '/')
			continue;
		...
		if (sscanf(line, "%[^:]: %[^\r\n]", w1, w2) < 2)
			continue;
		if ( !strcmpi(w1, "traspasa_hash") ) 
			traspasa_hash =  (unsigned int)atoi(w2);
		else if ( !strcmpi(w1, "log_hash") ) 
			log_hash =  (unsigned int)atoi(w2);
	...
	fclose(fp);
	ShowXRO("Finaliza lectura de la configuracion del hash check, conf/security_hash\n");
	return 0;
}
...
int do_init(int argc, char** argv)
{
	...
	/* ISAAC HASH CHECKER */
	hash_config_read((argc > 1) ? argv[1] : SECHASH_CONF_NAME);
