...
int _vShowMessage(enum msg_type flag, const char *string, va_list ap)
{
	...
	if(
	    ( ( flag == MSG_INFORMATION || flag == MSG_XRO ) && msg_silent&1) ||
	...
	switch (flag) {
		...
		case MSG_FATALERROR: //Bright Red (Fatal errors, abort(); if possible)
			...
			break;
		case MSG_XRO: //isaac
			strcat(prefix,CL_LT_CYAN"[XRO Info]"CL_RESET": ");
			break;
		default:
...
/* isaac... */
int ShowXRO(const char *string, ...) {
	int ret;
	va_list ap;
	va_start(ap, string);
	ret = _vShowMessage(MSG_XRO, string, ap);
	va_end(ap);
	return ret;
}
