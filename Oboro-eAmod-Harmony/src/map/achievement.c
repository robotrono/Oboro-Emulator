...
void achievement_complete(struct map_session_data* sd, struct achievement_data* ad)
{
	...
	// Announcement
	sprintf(output,msg_txt(909),ad->name);
 ...
 *------------------------------------------*/
static int achievement_validate(DBKey key, void *data, va_list ap)
{
	...
	if( (ad = (struct achievement_data*)data) == NULL || (sd = va_arg(ap,struct map_session_data*)) == NULL )
		return 0;
