...
int party_removemember(struct map_session_data* sd, int account_id, char* name)
{
	...
	// Antibug de canciones	[csnv]
	if(tsd = map_id2sd(p->party.member[i].char_id))
		party_checkdancingbuffs(tsd);
	//
...
int party_leave(struct map_session_data *sd)
{
	...
	/// Anti-bug de canciones [csnv]
	party_checkdancingbuffs(sd);
...
int party_member_withdraw(int party_id, int account_id, int char_id)
{
	...
	if( p )
	{
		...
		if( i < MAX_PARTY )
		{
			party_checkdancingbuffs(sd); // Bug canciones [csnv]
	...
	clif_party_info (p, NULL); // @sjob [csnv]
...
int party_broken(int party_id)
{
	...
	for( i = 0; i < MAX_PARTY; i++ )
	{
		if( p->data[i].sd!=NULL )
		{
			party_checkdancingbuffs( p->data[i].sd ); // Buffs de mierda [csnv]
...
/// Elimina buffs de canciones [csnv] pseudo-fix
void party_checkdancingbuffs(struct map_session_data *tsd)
{
	struct status_change *tsc;

	if (tsc = status_get_sc(&tsd->bl))
	{
		if(tsc->data[SC_DANCING])
			status_change_end(&tsd->bl, SC_DANCING, INVALID_TIMER);
		if(tsc->data[SC_DRUMBATTLE])
			status_change_end(&tsd->bl, SC_DRUMBATTLE, INVALID_TIMER);
		if(tsc->data[SC_NIBELUNGEN])	
			status_change_end(&tsd->bl, SC_NIBELUNGEN, INVALID_TIMER);
		if(tsc->data[SC_SIEGFRIED])	
			status_change_end(&tsd->bl, SC_SIEGFRIED, INVALID_TIMER);
		if(tsc->data[SC_ROKISWEIL])	
			status_change_end(&tsd->bl, SC_ROKISWEIL, INVALID_TIMER);
		if(tsc->data[SC_INTOABYSS])	
			status_change_end(&tsd->bl, SC_INTOABYSS, INVALID_TIMER);
	}
}
