...
unsigned char mail_setitem(struct map_session_data *sd, int idx, int amount)
{
	...
	{ // Item Transfer
		...
		sd->status.inventory[idx].card[0] == CARD0_CREATE &&
		(char_id = MakeDWord(sd->status.inventory[idx].card[2],sd->status.inventory[idx].card[3])) == battle_config.costume_reserved_char_id )
	...
	if( sd->mail.amount )
	{
		...
		if( battle_config.costume_reserved_char_id &&
			...
			(char_id = MakeDWord(sd->status.inventory[n].card[2],sd->status.inventory[n].card[3])) == battle_config.costume_reserved_char_id )
				return false; // Costume item [csnv]
