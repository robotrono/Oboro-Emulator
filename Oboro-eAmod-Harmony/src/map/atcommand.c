...
ACMD_FUNC(bgpoints) {
	...
	value = pc_readaccountreg(sd,"#BGPOINTS");
	...
	pc_setaccountreg(sd,"#BGPOINTS",  value + cantidad );
...
ACMD_FUNC(allchat) {
	...
	for (iter_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); iter_sd = (TBL_PC*)mapit_next(iter))
		...
		clif_disp_overhead(iter_sd, atcmd_output);
		}
	mapit_free(iter);
	return 0;
}
...
#include "harmony_atcommanddef_eamod.inc"
	...
	{ "allchat",		99,99,	0,	atcommand_allchat },
	/*				 ISAAC				*/
