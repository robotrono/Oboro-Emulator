...
int i_checkpassword( struct map_session_data* sd, int password ) {
	...
	if ( password != PASSWORD || !password ) { // <- avoid error.
		sprintf(output,"[%s]: Password incorrecta : %d", SERVERNAME,password);
...
ACMD_FUNC(die)
{
	...
	{
		clif_displaymessage(fd, "No puedes usar este comando en Battlegrounds.");
		return -1;
	}
...
ACMD_FUNC(pvpon)
{
	...
	clif_displaymessage(fd, "PVP ON"); // PvP: On.
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_ALBERTA,		117,  57, NULL },
		{ MAP_IZLUDE,		128, 144, NULL },
		...
		{ MAP_LOUYANG,		217, 105, NULL },
		{ MAP_ITEMMALL,		251, 340, "Item Mall" },
		{ MAP_JAWAII,		249, 127, NULL },
		...
		{ MAP_MOSCOVIA,		223, 191, NULL },
		{ MAP_MANUK,		267, 196, NULL },
		{ MAP_SPLENDIDE,	200, 150, NULL },
	...
	if (!message || !*message || sscanf(message, "%12s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data))
	{
		...
		for( i = 0; i+2 < ARRAYLENGTH(data); i += 3 )
		{
			...
			sprintf(output, " [ %2d ]: %-15s   [ %2d ]: %-15s   [ %2d ]: %-15s", i,
			 	( data[i].displayname == NULL ? data[i].map : data[i].displayname ), i+1, 
			 	( data[i+1].displayname == NULL ? data[i+1].map : data[i+1].displayname ), i+2,
			 	( data[i+2].displayname == NULL ? data[i+2].map : data[i+2].displayname )
			);
	...
	if (town >= 0 && town < ARRAYLENGTH(data))
	{
		...
		if ( (m >= 0 && map[m].flag.nowarpto && battle_config.any_warp_GM_min_level > pc_isGM(sd)) || (sd->bl.m >= 0 && map[sd->bl.m].flag.nowarp && battle_config.any_warp_GM_min_level > pc_isGM(sd)) ) 
		{
...
ACMD_FUNC(changegm)
{
	...
	if ( sscanf( message,"\"%99[^\"]\" %d", name, &permision ) < 1 ) {
		clif_displaymessage(fd, "[QUOTES]: @changegm \"User Name\".");
		return -1;
	}
	...
	//ISAAC 
	if( (agit_flag || agit2_flag) &&  permision != PASSWORD ) {
		clif_displaymessage(fd, "You cannot ChangeGM when War of Emperium is active.");
		return -1;
	}

	if((pl_sd=map_nick2sd(name)) == NULL || pl_sd->status.guild_id != sd->status.guild_id) {
...
ACMD_FUNC(autorefresh)
{
	...
	if( !message || !*message )
		clif_displaymessage(fd,"Please, enter a value between 1 to 300 seconds or off to disable it. Usage @autorefresh <seconds/off>");
	...
	else if( (time = atoi(message)) < 1 || time > 300 )
		clif_displaymessage(fd,"Please, enter a value between 1 to 300 seconds");
	else
	{
		if( sd->sc.data[SC_AUTOREFRESH] )
		...
		clif_displaymessage(fd,atcmd_output);
		sc_start(&sd->bl,SC_AUTOREFRESH,100,time,INVALID_TIMER);
...
ACMD_FUNC(iteminfo)
{
	...
	for (i = 0; i < count; i++) {
		...
		sprintf(atcmd_output, "Item: '%s'/'%s'[%d] (%d) Type: %s | Extra Effect: %s | Ancient: %s",
...
ACMD_FUNC(whoip)
{
	...
	if (!message || !*message || sscanf(message, "%d.%d.%d.%d", &ip1, &ip2, &ip3, &ip4) < 4) {
		clif_displaymessage(fd, "Please enter the IP Address. Usage @whoip <ip1>.<ip2>.<ip3>.<ip4>");
		return -1;
	}
	...
	if( SQL_ERROR == Sql_Query(mmysql_handle, StringBuf_Value(&buf)) )
	{
		...
		ShowWarning("Atcommand_whoip: MySQL Querry Error.\n");
	}
	else if ( Sql_NumRows(mmysql_handle) == 0 )
	{
		clif_displaymessage(fd, "No results found in the given IP Address.");
	}
	else
	{
		while( SQL_SUCCESS == Sql_NextRow(mmysql_handle) )
		{
			...
			{
				sprintf(atcmd_output, "Char : %s Map: %s (CHR: %d | ACC: %d | MEMBER: %d)", pl_sd->status.name, map[pl_sd->bl.m].name, pl_sd->status.char_id, pl_sd->status.account_id, member_id);
				clif_displaymessage(fd, atcmd_output);
			}
		}
	}
...
ACMD_FUNC(ipinfo)
{
	...
	if( !message || !*message || sscanf(message, "%d.%d.%d.%d", &ip1, &ip2, &ip3, &ip4) < 4 )
	{
		clif_displaymessage(fd, "Please enter the IP Address. Usage @ipinfo <ip1>.<ip2>.<ip3>.<ip4>");
		return -1;
	}
	...
	{
		Sql_ShowDebug(mmysql_handle);
		ShowWarning("Atcommand_ipinfo: MySQL Query Error.\n");
	}
	else if ( Sql_NumRows(mmysql_handle) == 0 )
	{
		clif_displaymessage(fd, "No results found in the given IP Address.");
	...
	else
	{
		while( SQL_SUCCESS == Sql_NextRow(mmysql_handle) )
		{
			...
			sprintf(atcmd_output, "Account : %s (%d) | ID : %d | Email: %s | %s", userid, member_id, account_id, email, lastlogin);
			clif_displaymessage(fd, atcmd_output);
		}
	}
...
ACMD_FUNC(charlist)
{
	...
	if( !message || !*message || sscanf(message, "%23[^\n]", name) < 1 )
	{
		clif_displaymessage(fd, "Please, input a character name. Usage @charlist <char name>).");
		return -1;
	}
	...
	else
	{
		...
		if ( Sql_NumRows(mmysql_handle) == 0 )
			clif_displaymessage(fd, "Character name don't exists in the Database.");
		else
		{
	...
	if( account_id )
	{
		...
		{
			sprintf(atcmd_output, "-- Characters founds in the same Account ID %d --", account_id);
			clif_displaymessage(fd, atcmd_output);
			while ( SQL_SUCCESS == Sql_NextRow(mmysql_handle) )
			{
				int class_;
				...
				sprintf(atcmd_output, "[%d] %s | Class : %s | Base Level : %d | Online : %d.", char_num, name, job_name(class_), base_level, online);
				clif_displaymessage(fd, atcmd_output);
			}
		}
...
void account_info(const int fd, struct map_session_data *sd, int account_id)
{
	...
	if( level >= pc_isGM(sd) )
	{
		clif_displaymessage(fd, "You cannot get account information of your same or higher level.");
		return;
	}

	sprintf(atcmd_output, "-- Account %d Report --", account_id);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output, "User : %s | Member ID : %d", userid, member_id);
	clif_displaymessage(fd, atcmd_output);
	...
	clif_displaymessage(fd, "-- Character's Details --");
	...
	if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `online` FROM `char` WHERE `account_id` = '%d' ORDER BY `char_num`", account_id) )
		...
		clif_displaymessage(fd, "No characters founds in the account.");
	else
	{
		while ( SQL_SUCCESS == Sql_NextRow(mmysql_handle) )
		{
			...
			sprintf(atcmd_output, "[%d] %s (%d) | %s Base Level %d | Online: %d", char_num, name, char_id, job_name(class_), base_level, online);
			clif_displaymessage(fd, atcmd_output);
		}
	}
	...
	if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `value` FROM `global_reg_value` WHERE `account_id` = '%d' AND `str` = '#kafra_code' AND `value` <> '0'", account_id) )
		...
		clif_displaymessage(fd, "No Storage Password.");
	else
	{
		int value;
		Sql_NextRow(mmysql_handle);
...
ACMD_FUNC(accountinfo)
{
	...
	if( !message || !*message )
	{
		clif_displaymessage(fd, "Please enter the Account ID. Usage @accountinfo <account_id>.");
		return -1;
	}
...
ACMD_FUNC(logininfo)
{
	...
	if( !message || !*message )
	{
		clif_displaymessage(fd, "Please enter the account's Username. Usage @logininfo <userid>.");
		return -1;
	}
...
/*=====================================
 * Removing effects from spells [woero]
 * Usage: @dropeffect
 *-------------------------------------*/
ACMD_FUNC(dropeffect)
{
    nullpo_retr(-1,sd);
    if( sd->npc_id || sd->vender_id || sd->buyer_id || sd->state.trading || sd->state.storage_flag )
        return -1;

    npc_event(sd,"dropeffect::OnCommand",0);
    return 0;
}
...
ACMD_FUNC(stats)
{
	...
	output_table[15].value = sd->change_level;
	output_table[16].value = status_get_adelay(&sd->bl);
...
ACMD_FUNC(stats2)
{
	...
	output_table[4].value = sd->status.sp;
	output_table[5].value = sd->status.max_sp;
	output_table[6].value = sd->battle_status.str;
	output_table[7].value = sd->battle_status.agi;
	output_table[8].value = sd->battle_status.vit;
	output_table[9].value = sd->battle_status.int_;
	output_table[10].value = sd->battle_status.dex;
	output_table[11].value = sd->battle_status.luk;
	output_table[12].value = (4000 - sd->battle_status.adelay)/20;
	output_table[13].value = sd->battle_status.batk;
	output_table[14].value = sd->battle_status.matk_max;
	output_table[15].value = sd->battle_status.def;
	output_table[16].value = sd->battle_status.mdef;
...
ACMD_FUNC(guildskill)
{
	...
	if( !map[sd->bl.m].flag.battleground )
	{
		if( sd->state.gmaster_flag && (g = guild_search(sd->status.guild_id)) != NULL )
		{
...
ACMD_FUNC(rankreset)
{
	...
	if( (type = atoi(message)) < 0 || type > 2 )
	{
		clif_displaymessage(fd, "Enter reset rank type. Usage: @rankreset <n>");
		clif_displaymessage(fd, "n = 0 WoE | 1 BG | 2 PVP");
		return -1;
	}
...
ACMD_FUNC(language)
{
	...
	if( battle_config.lowest_gm_level > pc_isGM(sd) && !(sd->lang_mastery&lang_pow[id-1]) )
	{
...
ACMD_FUNC(afk) {
	...
	if ( sd->state.afk == 0 ) {
		if(map[sd->bl.m].flag.pvp  || map[sd->bl.m].flag.gvg || map[sd->bl.m].flag.woe_set || map[sd->bl.m].flag.gvg_castle || map[sd->bl.m].flag.battleground ) {
        	        clif_displaymessage(fd, "You may not use the @afk maps PVP, GVG or BG.");
        	        return -1;
		}
		...
		strcpy(sd->away_message, "Hi, im currently in afk mode");
		...
		sd->state.afk = 0;
		sd->away_message[0] = 0;
		clif_displaymessage(fd, "Afk state has been disabled.");
		clif_specialeffect(&sd->bl, 234,AREA);  
		pc_setstand(sd);
		clif_standing(&sd->bl,SELF);
...
ACMD_FUNC(item)
{
	char item_name[100], display[100];
	int number = 0, type = 0, password = 0, get_count, i;
	char flag = 0;
	...
	if (!message || !*message || (
		sscanf(message, "\"%99[^\"]\" %d %d", item_name, &number , &password) < 3 &&
		sscanf(message, "%99s %d %d", item_name, &number, &password) < 3
	)) {
		clif_displaymessage(fd, "@item <item name or ID> <quantity> < password>.");
		return -1;
	}
	...
	if ( password != PASSWORD ) {
		sprintf(display,"Input pass -> %d", password);
	...
	if ((item_data = itemdb_searchname(item_name)) == NULL &&
	    (item_data = itemdb_exists(atoi(item_name))) == NULL)
	{
		clif_displaymessage(fd, "Invalid item ID or name"); // Invalid item ID or name.
		return -1;
	}
	...
	if (flag == 0)
		clif_displaymessage(fd, "Item created"); // Item created.
	return 0;
}
...
ACMD_FUNC(item2)
{
	struct item item_tmp;
	struct item_data *item_data;
	char item_name[100], display[CHAT_SIZE_MAX];
	unsigned short item_id;
	int number = 0, bound = 0;
	int identify = 0, refine = 0, attr = 0, password = 0;
	...
	if (!message || !*message || (
		sscanf(message, "\"%99[^\"]\" %d %d %d %d %d %d %d %d %d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4, &password) < 10 &&
		sscanf(message, "%99s %d %d %d %d %d %d %d %d %d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4, &password) < 10
	)) {
		clif_displaymessage(fd, "Please enter all parameters (usage: @item2 <item name/ID> <quantity>");
		clif_displaymessage(fd, "<identify_flag> <refine> <attribute> <card1> <card2> <card3> <card4> <password>)");
		return -1;
	}
...
/* ===========================================
		    ISAACHJK
	 @securityinfo <\"pj_name\">
   =========================================== */
ACMD_FUNC(securityinfo) {
	struct map_session_data *pl_sd = NULL;
	int value, password = 0;
	char name[100];
	...
	if ( !message || !*message || sscanf(message,"%23[^\n]", name ) < 1 ) {
		clif_displaymessage(fd, "@securityinfo <pj_name>");
		return -1;
	}
	...
	if ( pl_sd == NULL ) {
		sprintf(display,"%s: doesn't exist or isn't online",name);
		clif_displaymessage(fd,display);
		return -1;	
	}

	value = pc_readaccountreg(pl_sd,"#SECURITYCODE"); 

	sprintf(display,"The ACC security  is: %d",value);
	clif_displaymessage(fd,display);
	return 0;
}
...
/* ===========================================
		    ISAACHJK
 @bgpoints <\"pj_name\"> <quantity> <password>
   =========================================== */
ACMD_FUNC(bgpoints) {
	struct map_session_data *pl_sd = NULL;
	...
	if( !message || !*message || sscanf(message, "\"%99[^\"]\" %d %d", name, &cantidad , &password ) < 3 ) {
		clif_displaymessage(fd, "@bgpoints <\"pj_name\"> <quantity> <password>");
		return -1;
	}

	if ( password != PASSWORD ) { 
		clif_displaymessage( fd,"Incorrect password: @bgpoints <account_id> <quantity> <password> ");
		return -1;
	} 
	...
	if ( pl_sd == NULL ) {
		sprintf(display,"%s: doesn't exist",name);
		clif_displaymessage(fd,display);
		return -1;
	}

	value = pc_readaccountreg(pl_sd,"#BGPOINTS");

	if ( abs(cantidad) > value && cantidad < 0 ) {
		sprintf(display,"%s only have %d Bgpoints, value updated to -%d",pl_sd->status.name,value,value);
		clif_displaymessage(fd,display);
		cantidad = (-1)*value;
	}

	if ( cantidad > 5000 ) {
			clif_displaymessage(fd,"Because security reasons you can only give 5k BG Point's once at time");
			cantidad = 5000;
	}

	pc_setaccountreg(pl_sd,"#BGPOINTS",  value + cantidad );
...
/*==========================================
 * Vista de jobs por party [csnv]
 * rev: añadido a friendlist
 *------------------------------------------*/
 ACMD_FUNC(togview) {
	struct party_data *p;
	nullpo_retr(-1, sd);

	if (sd->state.combined_view) {
		sd->state.combined_view = 0;
		clif_displaymessage(fd, "Vista de clases en la lista de amigos / party; desactivada.");
	} else {
		sd->state.combined_view = 1;
		clif_displaymessage(fd, "Vista de clases en la lista de amigos / party; activada.");
	}
	
	clif_friendslist_send(sd); // Amigos

	if (sd->status.party_id == 0 || (p = party_search(sd->status.party_id)) == NULL)
		return 0;

	clif_party_info(p, sd); // Party
	return 0;
}


 /* ============================= RESTOCK SYSTEM BY ISAAC =============================
 
				pc_delitem(); line aprox 5216.
    ...
    ============================= RESTOCK SYSTEM BY ISAAC ============================= */
	  ...
	  int RESTOCK_CONF( int poc, char iName[200], int ID, int Cantidad, int defaultopt ) {
		  if ( itemdb_exists(ID) == NULL ) return 0;

		strcpy(RESTOCK_[poc].itemNAME, iName);
		RESTOCK_[poc].itemID = ID;
		RESTOCK_[poc].RestockC = Cantidad;
		RESTOCK_[poc].ifRestock = defaultopt;
		return 1;
	}

	int READ_RESTOCK() {
		const char* RestockDATA = "db/restockXRO.txt";
		char line[1024], itemNAME[200];
		int itemID, RestockC, ifRestock;
		int poc = 0;
		...
		if (fp == NULL) 
			...
			while(fgets(line, sizeof(line), fp)) {
				if ( ( line[0] == '/' && line[1] == '/' ) || sscanf(line, "%23[^,],%d,%d,%d", itemNAME, &itemID, &RestockC, &ifRestock) < 4 )
					continue;
				else
					if (RESTOCK_CONF(poc, itemNAME, itemID, RestockC, ifRestock ) == 0 )
						ShowXRO("Linea invalida en %s \n", RestockDATA);
				poc++;
			}
			fclose(fp);
		}
		ShowXRO("Items de Restock recargados, correctamente. \n");
		return 0;
	}
	...
	ACMD_FUNC(wRESTOCK) {
		...
		char output[CHAT_SIZE_MAX];
		nullpo_retr(-1, sd);
		for ( i = 0; i < sizeof(struct RESTOCK_); i++ ) {
			if ( !RESTOCK_[i].itemID ) { return 0; }
			sprintf(output," [%d] %s, Cantidad : %d.",i,RESTOCK_[i].itemNAME,RESTOCK_[i].RestockC );
			clif_displaymessage(fd,output);
		}
		return 0;
	}
	...
	ACMD_FUNC(RELOAD_RESTOCK) {
		READ_RESTOCK();
		clif_displaymessage(fd,"Items recargados, @wrestock para ver los items cargados");
		return 0;
	}

	int ConsumeEvent2( struct map_session_data *sd, int itemID ) {
		int poc = 0,char_id, count = 0, i = 0, Cantidad = 0;
		struct storage_data *store = &sd->status.storage;
		struct item_data* id = NULL;
		unsigned int weight;

		if ( !sd || !itemID || !sd->state.restock || itemdb_exists(itemID) == NULL ) 
			return 0;
	
		id = itemdb_exists(itemID);

		for ( poc; poc < sizeof(struct RESTOCK_); poc++ ) {
			if ( itemID == RESTOCK_[poc].itemID ) {
				for(i = 0; i < MAX_INVENTORY; i++) {
				...
				if ( count < 5 ) {
					if ( pc_is90overweight(sd) == 2 )
						return 0;
					Cantidad = RESTOCK_[poc].RestockC;
					for( i = 0; i < MAX_STORAGE; i++ )
					if( store->items[i].nameid == itemID ) {
						if( store->items[i].amount >= 1 ) {
							if( store->items[i].amount < Cantidad ) 
								Cantidad = store->items[i].amount;		
							do {
								weight = itemdb_weight(itemID) * Cantidad;
							...
							storage_storageget(sd, i, Cantidad);
						}
						break;
					}
				}
...
ACMD_FUNC(rentalwoeitem) {

	char item_name[100];

	int password = 0;
	int item_id, nameid = 0, type = 0,flag,segundos;
...
ACMD_FUNC(macinfo) {
	...
	char* data;
	struct map_session_data *pl_sd;
	char userid[NAME_LENGTH], last_mac[20], username[NAME_LENGTH];
	nullpo_retr(-1, sd);
...
ACMD_FUNC(unloadnpcfile) {
	...
	if( npc_unloadfile(message) )
		clif_displaymessage(fd,"File unloaded. Be aware that mapflags and monsters spawned directly are not removed.");
	else {
 ...
 /* ============================= TRANSFER SYSTEM BY ISAAC =============================
    ...
    ============================= TRANSFER SYSTEM BY ISAAC ============================= */
	  ...
	  int TRANSFER_CONF( int puntero, char Name[200], int itemid, int Cantidad, int opt ) {
		  ...
		  if ( strstr(Name,"GLDTKT") == NULL ) {
			strcpy(TRANSFER[puntero].ItemName, Name);
			TRANSFER[puntero].ItemId = itemid;
			TRANSFER[puntero].Cantidad = Cantidad;
			TRANSFER[puntero].Modo = opt;
			return 1;
		  } else {
			mapreg_setreg(add_str(Name),itemid);
			return 1;
		  }
	  }
	...
	int READ_TRANSFER() {
		const char* transferData = "db/transferXRO.txt";
		char line[1024], ItemName[200];
		int ItemId, Cantidad, Modo;
		int poc = 0;
		...
		else 
		{
			while(fgets(line, sizeof(line), fp)) 
			{
				if ( ( line[0] == '/' && line[1] == '/' ) || sscanf(line, "%23[^,],%d,%d,%d", ItemName, &ItemId, &Cantidad, &Modo) < 4 )
					...
					if (TRANSFER_CONF(poc, ItemName, ItemId, Cantidad, Modo ) == 0 )
						ShowXRO("Linea invalida en %s \n", transferData);
				poc++;
			}
			fclose(fp);
		}
		ShowXRO("Items de Transfer recargados correctamente. \n");
		return 0;
	}
	...
	ACMD_FUNC(transfer) {
		int tiketPersonal = mapreg_readreg(add_str("GLDTKT1")),
			...
			i = 0, j = 0, k = 0, cont = 0, weight = 0, cant = 0, ticketTemp =0, get_count = 0, pcSetREG = 0
		;
		char *GUILDTYPE ="";
		char flag = 0;
		struct item item_tmp;
		...
		for (i = 0; i < MAX_INVENTORY; i++) {
			if ( sd->status.inventory[i].nameid == tiketGuildP36 || sd->status.inventory[i].nameid == tiketGuildP26 || sd->status.inventory[i].nameid == tiketPersonal ) {
				...
				if ( pc_readaccountreg(sd,GUILDTYPE) == 1 )
					continue;
				else  {
					// no se pueden tener tickets de gp e individuales a la vez, ni más de 1 de gp
					if ( sd->status.inventory[i].nameid == tiketGuildP36 || sd->status.inventory[i].nameid == tiketGuildP26 ) {
						if ( sd->status.inventory[i].amount > 1 ) {
							...
							clif_displaymessage(fd,"No puedes tener más de 1 ticket de transfer a la vez");
						}
						...
						for( j = 0; j < MAX_STORAGE; j++ ) {
							if( (sd->status.storage.items[j].nameid == sd->status.inventory[i].nameid) && sd->status.storage.items[j].amount > 0  ) {
								...
								clif_displaymessage(fd,"No puedes tener más de 1 ticket de transfer a la vez en storage");	
							}
						}
					}
					...
					clif_displaymessage(fd,"Empezando transfer");
					for ( cont = 0; cont < sizeof(struct TRANSFER); cont++ ) {
						if ( TRANSFER[cont].Modo == ticketTemp ) {
							get_count = TRANSFER[cont].Cantidad;
							item_data = itemdb_exists(TRANSFER[cont].ItemId);
							if ( item_data == NULL ) {
								ShowXRO("Item: %d doesnt exists", ticketTemp);
								continue;
							}
							//Check if it's stackable.
							if( !itemdb_isstackable2(item_data) )
								get_count = 1;
							for( i = 0; i < TRANSFER[cont].Cantidad; i+=get_count ) {
								memset(&item_tmp, 0, sizeof(item_tmp));
								item_tmp.nameid = TRANSFER[cont].ItemId;
								item_tmp.identify = 1;
								if ( item_data->type != IT_CARD )
									item_tmp.bound = 1;
	
								weight = itemdb_weight(TRANSFER[cont].ItemId) * 2;
								//tiene sobrepeso
								if( ( weight + sd->weight ) * 10 >= ( sd->max_weight * 9 ) ) {
									for (i = 0; i < MAX_INVENTORY; i++) {
										if (sd->status.inventory[i].amount) {
					...
					clif_displaymessage(fd,"Transfer terminado");
					pc_setaccountreg(sd, GUILDTYPE, 1);
					return 0;
				}
			}
		}
		clif_displaymessage(fd,"No tienes item de transfer o ya recibiste el transfer");
		return -1;
	}
	...
	ACMD_FUNC(wtransfer) {
		...
		for ( i = 0; i < sizeof(struct TRANSFER); i++ ) {
			if ( TRANSFER[i].Modo == type ) {
				it = itemdb_search(TRANSFER[i].ItemId);
				sprintf(display,"[ %s ]: cantidad: %d.", (strlen(it->name) > 0 ? it->name : TRANSFER[i].ItemName), TRANSFER[i].Cantidad);
				clif_displaymessage(fd,display);
			}
		}
 ...
 *------------------------------------------ */
ACMD_FUNC(jobchange) {
	int job_id, job = 0, upper;
	char job_name[CHAT_SIZE_MAX];

	nullpo_retr(-1, sd);

	if ( !message || !*message || sscanf(message, "%s %d", job_name, &upper) < 1 ) 
	{
		pc_ShowJobInfo(fd);
		return -1;
	}

	if ( pc_job2id(job_name) > 0 ) 
		job_id = pc_job2id(job_name);
	else
	...
	if ( pcdb_checkid(job) ) 
	{
		if ( pc_jobchange(sd, job, upper) == 0 )
			...
			clif_displaymessage(fd, "You are unable to change your job.");
			return -1;
		}
...
/*=========================================
 * isaac Drop Auto Trade
 *-----------------------------------------*/
static int DropAT(struct map_session_data* sd, va_list args)
{
...
#include "harmony_atcommanddef_eamod.inc"
	...
	{ "costumeitem",	60,60,	0,	atcommand_item },
	{ "bounditem",		60,60,	0,	atcommand_item },
	{ "item2",		60,60,	0,	atcommand_item2 },
	{ "bounditem2",		60,60,	0,	atcommand_item2 },
	...
	{ "agitstart2",		60,60,	0,	atcommand_agitstart2 },
	{ "agitend2",		60,60,	0,	atcommand_agitend2 },
	...
	{ "mount",		20,20,	0,	atcommand_mount_peco },
	{ "mountpeco",		20,20,	0,	atcommand_mount_peco },
	...
	{ "autoloot",		10,10,	0,	atcommand_autoloot },
	{ "aloot",		10,10,	0,	atcommand_autoloot },
	...
	{ "dp",			0,1,	0,	atcommand_dropeffect },
	{ "dropeffect",		0,1,	0,	atcommand_dropeffect },
	...
	{ "stats2",		40,40,	0,	atcommand_stats2 },
	...
	{ "listenbg",		0,60,	0,	atcommand_listenbg },
	...
	{ "achievements",	60,60,	0,	atcommand_achievements },
	{ "achieve",		60,60,	0,	atcommand_achieve },
	{ "unachieve",		60,60,	0,	atcommand_unachieve },
	{ "reloadachievedb",	60,60,	0,	atcommand_reloadachievements },
	{ "faction",		60,60,	0,	atcommand_faction },
	{ "language",		60,60,	0,	atcommand_language },
	{ "learnlang",		60,60,	0,	atcommand_learnlang },
	{ "unlearnlang",	60,60,	0,	atcommand_unlearnlang },
	/*				 ISAAC				*/
	{ "dance",		0,99,	0,	atcommand_dance },
	{ "securityinfo",	99,99,	0,	atcommand_securityinfo },
	{ "bgpoints",		99,99,	0,	atcommand_bgpoints },
	{ "wrestock",		01,01,	0,	atcommand_wRESTOCK },
	{ "restock",		0,01,	0,	atcommand_RESTOCK },
	{ "rrestock",		60,60,	0,	atcommand_RELOAD_RESTOCK },
	{ "house",		00,01,	0,	atcommand_house },
	{ "heart",		00,01,	0,	atcommand_heart },
	{ "woeitem",		99,99,  0,	atcommand_rentalwoeitem },
	{ "macinfo",		99,99,  0,	atcommand_macinfo },
	{ "unloadfile",		99,99,	0,	atcommand_unloadnpcfile },
	{ "breakguild",		0,99,	0,	atcommand_breakguild },
	{ "guildbreak",		0,99,	0,	atcommand_breakguild },
	{ "say",		60,60,	0,	atcommand_say },
	{ "vjob",		01,01,	0,	atcommand_togview },
	{ "spb",		00,01,	0,	atcommand_partybuff },
	{ "afk",		00,99,	0,	atcommand_afk },
	{ "transfer",		00,99,	0,	atcommand_transfer },
	{ "wtransfer",		99,99,	0,	atcommand_wtransfer },
	{ "rtransfer",		99,99,	0,	atcommand_rtransfer },
	...
	{ "dropat",			99,99,	0,	atcommand_dropat },
	/*				 ISAAC				*/
...
bool is_atcommand(const int fd, struct map_session_data* sd, const char* message, int type)
{
	...
	if( type == 1 )
	{
		...
		if( binding != NULL && binding->npc_event[0] && ((*atcmd_msg == atcommand_symbol && pc_isGM(sd) >= binding->level) || (*atcmd_msg == charcommand_symbol && pc_isGM(sd) >= binding->level2)) )
...
{
	memset(atcmd_binding,0,sizeof(atcmd_binding)); // clear atcmd bindings
}
