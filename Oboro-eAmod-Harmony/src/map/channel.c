...
void channel_join(struct map_session_data *sd, const char* name, const char* pass, bool invite)
{
	...
	if( !invite )
	{
		...
		if( cd->type == CHN_GAMEMASTER && pc_isGM(sd) < battle_config.lowest_gm_level )
		{
			clif_displaymessage(sd->fd, msg_txt(903));
			return;
		}
	}
	...
	sprintf(output, msg_txt(910), sd->status.name, cd->name);
...
int channel_invite_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	{
		sprintf(output, msg_txt(901), cd->name, sd->status.name);
		clif_channel_message(cd, output, -1);
	}

	if( sd )
	{
		clif_disp_onlyself(sd, msg_txt(902), strlen(msg_txt(902)));
...
void channel_message(struct map_session_data *sd, const char* channel, const char* message)
{
	...
	{ // Channel commands
		...
		else if( cd->op != sd->bl.id && pc_isGM(sd) < battle_config.gm_channel_operator )
			return;
		...
		{ // Invite a User to the Channel
			...
			if( (p_sd = map_nick2sd(option_text)) == NULL )
				...
				clif_displaymessage(sd->fd, msg_txt(900));
...
static int channel_db_final(DBKey key, void *data, va_list ap)
{
	struct channel_data *cd = (struct channel_data *)data;
	db_destroy(cd->users_db);
	return 0;
}
