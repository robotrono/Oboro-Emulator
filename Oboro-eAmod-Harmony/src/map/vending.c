...
void vending_openvending(struct map_session_data* sd, const char* message, bool flag, const uint8* data, int count)
{
	...
	for( j = 0; j < count; j++ )
	{
		...
		||  ( battle_config.costume_reserved_char_id && sd->status.cart[index].card[0] == CARD0_CREATE && (char_id = MakeDWord(sd->status.cart[index].card[2],sd->status.cart[index].card[3])) == battle_config.costume_reserved_char_id ) // Costume item [csnv]
