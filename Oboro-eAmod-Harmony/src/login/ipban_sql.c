...
// misma de arriba mas estricta ^ isaac...
void ipban_log_as_grf_integrity(uint32 ip)
{
	uint8* p = (uint8*)&ip;
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s`(`list`,`btime`,`rtime`,`reason`) VALUES ('%u.%u.%u.*', NOW() , NOW() +  INTERVAL 4 DAY ,'Traspasa integridad de grf')",
		ipban_table, p[3], p[2], p[1], login_config.dynamic_pass_failure_ban_duration) )
		Sql_ShowDebug(sql_handle);
}
